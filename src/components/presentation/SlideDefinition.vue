...
<MathSlideWrapper :title="slide.title || 'Definitie'">
    <PedagogyPanel variant="theory" :title="slide.term || 'Definitie'" icon="definition">
        ...
        <p v-html="slide.explanation" class="text-slide-heading text-slate-700 font-medium"></p>
        
        <ul v-if="slide.conditions && slide.conditions.length" class="mt-6 space-y-3">
            <li v-for="(condition, idx) in slide.conditions" :key="idx" 
                class="text-slide-body text-slate-600 flex items-start gap-3 bg-slate-50 border border-slate-200 p-4 rounded-lg">
                <div class="w-6 h-6 rounded-full bg-white border border-slate-300 flex items-center justify-center flex-shrink-0 mt-0.5 shadow-sm">
                    <span class="w-2.5 h-2.5 rounded-full bg-presentation"></span>
                </div>
    ...
    <div v-if="slide.examples && slide.examples.length" class="grid grid-cols-2 gap-6 mt-6">
        <div v-for="(example, idx) in slide.examples" :key="idx"
            ...
            <p class="text-slide-heading font-bold text-slate-800 mb-3" v-html="example.formula"></p>
            <p class="text-slide-body text-slate-500 leading-relaxed font-medium" v-html="example.note"></p>
        </div>
    </div>
    ...
    <div v-if="slide.relatedTerms && slide.relatedTerms.length" class="flex flex-wrap gap-3 mt-6 pt-6 border-t border-slate-100">
        <span v-for="(term, idx) in slide.relatedTerms" :key="idx"
              class="px-4 py-2 bg-white border border-slate-200 shadow-sm rounded-full text-slide-body text-ink-soft font-bold tracking-wide uppercase cursor-default">
            {{ term }}
        </span>
    </div>
</MathSlideWrapper>
