<template>
  ...
  <div class="w-full h-full flex flex-col bg-paper">
    <header class="px-16 py-8 shrink-0" style="background: var(--color-panel); border-bottom: 3px solid var(--color-presentation);">
      <div class="flex items-center gap-3 mb-3">
        ...
        <span class="fullscreen-label" style="color: var(--color-presentation);">Afspraken</span>
      </div>
      <h1 class="text-5xl font-bold text-slate-900 leading-tight tracking-tight" v-html="slide.title"></h1>
      <p v-if="slide.subtitle" class="text-xl text-slate-700 mt-2" v-html="slide.subtitle"></p>
    </header>
    ...
    <div class="flex-1 px-16 py-8 flex flex-col justify-between overflow-y-auto">
      <div class="grid grid-cols-4 gap-6 my-auto">
        <div
          ...
          :key="index"
          class="flex flex-col items-start text-left py-8 px-6 bg-white rounded-card relative"
          style="border: 2px solid var(--color-line-strong);"
        >
          <!-- Nummer: functioneel, geen decoratie -->
          <span class="font-mono text-sm font-bold text-slate-600 mb-4">{{ String(index + 1).padStart(2, '0') }}</span>

          <component v-if="rule.icon" :is="getIcon(rule.icon)" weight="bold" class="text-4xl mb-4"
                     :style="{ color: rule.highlight ? 'var(--color-presentation)' : 'var(--color-ink-soft)' }" />

          <div class="text-lg text-slate-800 leading-relaxed" v-html="rule.text"></div>
        </div>
      </div>

      <div v-if="slide.note || slide.cursusLink" class="rounded-card p-6 flex items-start gap-4 mt-auto" style="background: var(--color-panel); border: 2px solid var(--color-line);">
