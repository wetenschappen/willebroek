...
<div class="absolute inset-0 bg-paper flex items-stretch">
    <div class="flex-1 flex flex-col items-start justify-center px-16 py-12">
        <div class="w-16 h-2 mb-8 slide-anchor"></div>
        <h3 class="text-slide-title font-bold text-slate-900 mb-6 tracking-tight" v-html="slide.title"></h3>
        <p v-if="slide.subtitle" class="text-slide-heading text-slate-700 leading-relaxed" v-html="slide.subtitle"></p>
        <p v-if="slide.credit" class="mt-8 slide-meta">{{ slide.credit }}</p>
    </div>
