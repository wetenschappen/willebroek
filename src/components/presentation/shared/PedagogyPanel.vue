...
const themeColors = computed(() => {
  ...
  switch(props.variant) {
    ...
    case 'exercise':
      return { 
        accent: 'bg-presentation text-white',
        text: 'text-presentation',
        border: 'border-presentation bg-presentation-soft'
      }
    case 'theory':
    case 'example':
    default:
  ...
  <div class="border rounded-xl p-8 mb-6 shadow-sm" :class="themeColors.border">
    ...
    <div v-if="title" class="flex items-center gap-3 border-b border-slate-100 pb-4 mb-6">
      <div class="w-9 h-9 rounded-lg flex items-center justify-center shadow-sm shrink-0" :class="themeColors.accent">
        <component :is="resolvedIcon" weight="bold" class="text-slide-body" />
      </div>
      <span class="text-slide-body font-bold tracking-wider uppercase text-slate-800" v-html="title"></span>
    </div>

    <!-- Panel Content -->
    <div class="text-slide-body text-slate-700 leading-relaxed">
      <slot />
    </div>
  </div>
</template>
