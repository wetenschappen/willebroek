      ...
      <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 md:flex z-10">
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <details class="bg-amber-50 rounded-xl p-4 border border-amber-200 mb-4">
            ...
            <div class="mt-3 text-sm space-y-2">
              <p class="flex items-center gap-2"><PhCheckCircle weight="bold" class="w-4 h-4 text-amber-500 shrink-0" /> Stel: de basket staat op (6, 3). We kiezen p = 6 (top horizontaal bij basket).</p>
              <p class="flex items-center gap-2"><PhCheckCircle weight="bold" class="w-4 h-4 text-amber-500 shrink-0" /> Kies q = 4 (top iets hoger dan basket, zodat de bal van boven komt).</p>
              <p class="flex items-center gap-2"><PhCheckCircle weight="bold" class="w-4 h-4 text-amber-500 shrink-0" /> Kies a = -0.5 (parabool opent naar beneden, realistische boog).</p>
              <p class="flex items-center gap-2"><PhCheckCircle weight="bold" class="w-4 h-4 text-amber-500 shrink-0" /> Resultaat: y = -0.5(x - 6)^2 + 4 — de bal gaat door de basket!</p>
            </div>
          </details>

          <!-- Prediction Gate -->
          <div v-if="!predictionMade" class="bg-amber-50 rounded-xl p-4 border border-amber-200 mb-4">
            <h4 class="font-bold text-amber-800 text-sm mb-2 flex items-center gap-2"><PhQuestion weight="bold" class="w-4 h-4" /> Voorspel!</h4>
            <p class="text-xs text-amber-600 mb-3">{{ currentInternalLevel === 0 ? 'Wat denk je? Hoe makkelijk wordt dit?' : currentInternalLevel === 1 ? 'Wat moet je aanpassen voor een verdere basket?' : 'Hoe ga je over de verdediger heen komen?' }}</p>
            <div class="space-y-2">
              <label v-for="opt in predictionOptions" :key="opt.id" class="flex items-center gap-2 p-2 rounded-lg bg-white border cursor-pointer text-xs"
                :class="predictionChoice === opt.id ? 'border-indigo-500 bg-amber-100' : 'border-slate-200'">
                <input type="radio" :value="opt.id" v-model="predictionChoice" class="accent-amber-500" />
                {{ opt.text }}
              </label>
            </div>
            <button @click="confirmPrediction" :disabled="!predictionChoice"
              class="mt-3 w-full py-2 bg-indigo-600 text-white font-bold rounded-lg text-sm disabled:opacity-50 hover:bg-amber-500 transition-all">
              Bevestig Voorspelling
            </button>
          </div>

          <!-- Prediction result -->
          <div v-if="predictionResult && isCorrect" class="bg-amber-50 rounded-xl p-3 border border-amber-200 mb-4 text-xs">
            <p class="font-bold text-amber-800">{{ predictionResult }}</p>
          </div>
          ...
          <div v-if="predictionMade" class="space-y-4">
            <div class="bg-amber-50 p-4 border border-amber-200 rounded-xl">
              ...
              <div class="font-mono font-black text-xl text-slate-700 bg-white p-3 rounded-lg border text-center tabular-nums">
                y = <span class="text-amber-600">{{ aVal.toFixed(2) }}</span>(x - <span class="text-blue-600">{{ pVal }}</span>)² + <span class="text-amber-600">{{ qVal }}</span>
              </div>
              ...
              <div class="mt-4 space-y-3">
                ...
                <div>
                  <label class="text-xs font-bold text-amber-700 flex justify-between"><span>q (Hoogte)</span><span class="tabular-nums">{{ qVal }}</span></label>
            ...
            <!-- Waarom werkt dit -->
            <div v-if="showWhy" class="bg-amber-50 rounded-xl p-4 border border-amber-200 animate-fadeIn">
              <h4 class="font-bold text-amber-800 text-sm mb-2">Waarom werkt dit?</h4>
              <p v-for="(line, i) in whyText" :key="i" class="text-xs text-amber-700 mb-1">{{ line }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="p-4 bg-slate-50 border-t border-slate-200 shrink-0">
          ...
          <div v-if="feedback.text" class="flex items-start gap-3 p-3 mb-3 text-sm rounded-lg animate-fadeIn"
            :class="{'bg-amber-50 text-amber-800': feedback.type === 'success', 'bg-red-100 text-red-800': feedback.type === 'error', 'bg-blue-100 text-blue-800': feedback.type === 'info'}">
          ...
          <div v-else-if="!showReflection && !showLPD" class="flex items-center gap-3">
            ...
            <button v-else @click="handleNext" class="flex-1 py-3 font-bold text-white rounded-lg shadow-md bg-amber-600 hover:bg-amber-500 flex items-center justify-center gap-2 animate-fadeIn">
          ...
          <div v-if="showReflection" class="space-y-3">
            <h4 class="font-bold text-amber-800 text-sm">Reflectie</h4>
            <p class="text-xs text-amber-600">Beantwoord deze vragen om je begrip te verdiepen.</p>
            <div v-for="(q, i) in reflectionQuestions" :key="i" class="space-y-1">
              <label class="text-xs font-medium text-slate-700">{{ q }}</label>
              <textarea v-model="reflectionAnswers[i]" rows="2" class="w-full text-sm p-2 border border-amber-200 rounded-lg focus:border-indigo-500 bg-white resize-none"
                :placeholder="'Typ je antwoord (min 10 tekens)...'"></textarea>
            </div>
            <button @click="checkReflection" :disabled="!allReflectionsDone"
              class="w-full py-2 bg-indigo-600 text-white font-bold rounded-lg disabled:opacity-50 hover:bg-amber-500 transition-all text-sm">
