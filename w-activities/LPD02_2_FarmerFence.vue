      ...
      <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 md:flex z-10">
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <details class="bg-amber-50 rounded-xl p-4 border border-amber-200 mb-4">
            ...
            <div class="mt-3 text-sm space-y-2">
              <p class="flex items-center gap-2"><PhCheckCircle weight="bold" class="w-4 h-4 text-amber-500 shrink-0" /> Stel: 20m hek tegen de rivier. Formule: opp = x (20 - 2x).</p>
              <p class="flex items-center gap-2"><PhCheckCircle weight="bold" class="w-4 h-4 text-amber-500 shrink-0" /> Probeer x = 4: opp = 4 x 12 = 48 m^2.</p>
              <p class="flex items-center gap-2"><PhCheckCircle weight="bold" class="w-4 h-4 text-amber-500 shrink-0" /> Probeer x = 5: opp = 5 x 10 = 50 m^2. Dat is al beter!</p>
              <p class="flex items-center gap-2"><PhCheckCircle weight="bold" class="w-4 h-4 text-amber-500 shrink-0" /> Probeer x = 6: opp = 6 x 8 = 48 m^2. Daalt terug. Maximum = 50 m^2.</p>
            </div>
          </details>
          ...
          <div v-if="!predictionMade" class="bg-amber-50 rounded-xl p-4 border border-amber-200 mb-4">
            ...
            <p class="text-xs text-amber-600 mb-3">Hoe ga je de maximale oppervlakte vinden?</p>
            <div class="space-y-2">
        ...
        <div class="p-4 bg-slate-50 border-t border-slate-200 shrink-0">
          ...
          <div v-else-if="!showReflection && !showLPD" class="flex items-center gap-3">
            ...
            <button v-else @click="handleNext" class="flex-1 py-3 font-bold text-white rounded-lg bg-amber-600 hover:bg-amber-500">Reflecteer <PhArrowRight weight="bold" class="inline" /></button>
          </div>
          ...
          <div v-if="showReflection" class="space-y-3">
            ...
            <div v-for="(q, i) in reflectionQuestions" :key="i" class="space-y-1">
              ...
              <textarea v-model="reflectionAnswers[i]" rows="2" class="w-full text-sm p-2 border border-amber-200 rounded-lg bg-white resize-none" placeholder="Typ je antwoord (min 10 tekens)..."></textarea>
            </div>
