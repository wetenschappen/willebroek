...
function checkPrediction() {
  ...
  if (predM === data.targetM && predQ === data.targetQ) {
    ...
    if (predM === data.targetQ && predQ === data.targetM) {
      errorDetected.value = data.errorSwap
    } else if (predM === -data.targetM || predQ === -data.targetQ) {
      errorDetected.value = data.errorWrongSign
    } else if (predM === data.targetM && predQ !== data.targetQ) {
      errorDetected.value = `m = ${predM} is juist! Maar q klopt niet. q is de y-waarde bij x = 0.`
    } else if (predQ === data.targetQ && predM !== data.targetM) {
      errorDetected.value = `q = ${predQ} is juist! Maar m klopt niet. Hoeveel verandert de lijn per stap?`
    } else {
      errorDetected.value = 'Voorspelling klopt niet. Controleer je berekening.'
    }
...
function checkReflection() {
  ...
  const conceptWords = ["grafiek", "functie", "lineair", "helling", "snijpunt", "as", "m", "q", "formule"]
  const reasonWords = ["omdat", "dus", "daarom", "want", "als", "dan", "doordat", "hoe", "hoeveel"]
...
function handleKeydown(e) {
  ...
  if (e.key === 'Enter' && showPrediction.value) checkPrediction()
  if (e.key === 'Enter' && showReflection.value) checkReflection()
}
      ...
      <main class="flex flex-1 overflow-hidden">
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <!-- Prediction Gate -->
            <div v-if="showPrediction" class="p-4 bg-amber-50 border border-amber-300 rounded-xl mb-4">
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            <div v-if="feedback.text"
                 ...
                 :class="{
                   ...
                   'bg-slate-100 text-slate-700': feedback.type === 'error',
                 }">
               <component :is="feedback.type === 'error' ? PhWarningCircle : (feedback.type === 'info' ? PhLightbulb : PhCheckCircle)" class="w-5 h-5 shrink-0 mt-0.5" weight="fill" />
            ...
            <div class="flex items-center gap-4">
              ...
              <button v-if="showPrediction" @click="showHint"
              ...
              <button v-if="showPrediction" @click="checkPrediction"
