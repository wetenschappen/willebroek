...
import {
  PhX, PhCheckCircle, PhWarningCircle, PhArrowRight, PhChartBar, PhArrowClockwise, PhTable, PhSortAscending, PhLightbulb, PhQuestion, PhTrendUp, PhBrain
...
const Bloom = computed(() => {
  return levelLabels[currentInternalLevel.value] || 'Toepassen'
})
...
function selectOption(opt) {
  ...
  if (currentSubLevel.value === 0 && opt === currentLevelData.value.targetVal) {
    ...
    if (lvl === 0 && opt === 'Staafdiagram' && target === 'Lijndiagram') {
      errorDetected.value = 'Let op: een staafdiagram toont afzonderlijke categorieën. Voor een trend over tijd heb je een lijndiagram nodig.'
    } else if (lvl === 2 && opt === 'Staafdiagram' && target === 'Lijndiagram') {
      errorDetected.value = 'Bij een doorlopende tijdsas is een lijndiagram vaak beter — de lijn toont het patroon.'
    } else if (lvl === 2 && opt === 'Cirkeldiagram' && target === 'Staafdiagram') {
      errorDetected.value = 'Een cirkeldiagram toont delen van een geheel (procenten). Voor het vergelijken van waarden gebruik je een staafdiagram.'
    } else if (lvl === 2 && opt === 'Lijndiagram' && target === 'Staafdiagram') {
      errorDetected.value = 'Een lijndiagram toont een trend. Voor afzonderlijke categorieën vergelijken is een staafdiagram duidelijker.'
    } else {
      errorDetected.value = 'Niet correct. Kijk goed naar de data of grafiek.'
    }
    feedback.value = { type: 'error', text: hintText }
  }
}
...
function checkReflection() {
  ...
  const conceptWords = ['staafdiagram', 'lijndiagram', 'grafiek', 'data', 'vergelijken', 'trend', 'categorie', 'tijd', 'as', 'staaf', 'lijn', 'neerslag', 'maand', 'waarde', 'verschil']
  const reasonWords = ['omdat', 'dus', 'daarom', 'want', 'als', 'dan', 'doordat', 'hierdoor', 'beter', 'duidelijker', 'geschikt', 'past']
  ...
  else if (!hasConcept) feedback.value = { type: 'error', text: 'Probeer vaktaal te gebruiken, zoals "staafdiagram", "lijndiagram" of "trend".' }
  else feedback.value = { type: 'error', text: 'Leg ook uit WAAROM een bepaald type geschikt is, gebruik woorden zoals "omdat" of "daarom".' }
  return false
}
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div class="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-xl">
              <button @click="showWorkedExample = !showWorkedExample" class="flex items-center gap-2 text-sm font-bold text-amber-700 w-full text-left">
                <PhLightbulb weight="fill" class="w-4 h-4" /> Voorbeeld: Staafdiagram vs Lijndiagram
              </button>
              <div v-if="showWorkedExample" class="mt-3 text-sm text-amber-800 space-y-2 animate-fadeIn">
                <p><strong>Stap 1:</strong> Een staafdiagram toont elke maand als aparte staaf — ideaal om afzonderlijke waarden te vergelijken.</p>
                <p><strong>Stap 2:</strong> Een lijndiagram verbindt de maanden met een lijn — toont de trend en ontwikkeling over tijd.</p>
                <p><strong>Stap 3:</strong> Kies het diagramtype op basis van je onderzoeksvraag: vergelijken → staafdiagram, trend → lijndiagram.</p>
                <p class="font-bold text-amber-600">Beide gebruiken dezelfde data, maar vertellen een ander verhaal!</p>
              </div>
            </div>
            ...
            <!-- Why Phase -->
            <div v-if="showWhy && !showReflection" class="p-4 mt-4 border border-indigo-200 bg-indigo-50 rounded-xl animate-fadeIn">
              ...
              <MathText :content="currentLevelData?.whyText || ''" class="text-xs text-indigo-600 leading-relaxed" />
            ...
            <!-- Reflection Phase -->
            <div v-if="showReflection || phase === 'reflect'" class="p-4 mt-4 border border-indigo-200 bg-indigo-50 rounded-xl shadow-inner animate-fadeIn">
              <div class="flex items-center gap-2 mb-2">
                <PhBrain weight="fill" class="w-5 h-5 text-indigo-600" />
                <label class="text-sm font-bold text-indigo-700">Reflectie</label>
              </div>
              <p class="text-xs text-indigo-600 mb-3">{{ currentLevelData?.reflectionQuestion }}</p>
              <textarea v-model="reflectionAnswer" :disabled="reflectionDone"
                        class="w-full p-3 border-2 border-indigo-200 rounded-lg text-sm text-indigo-800 bg-white focus:ring-indigo-500 focus:border-indigo-500 resize-none"
                        rows="3" placeholder="Schrijf je antwoord in volledige zinnen..."></textarea>
              <button v-if="!reflectionDone" @click="checkReflection()"
                      class="w-full mt-3 py-3 font-bold text-white bg-indigo-600 hover:bg-indigo-500 rounded-lg transition-all active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-indigo-500">
                Bevestig reflectie
              </button>
            </div>
          </div>
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
             <div v-if="feedback.text" class="flex items-start gap-4 p-4 mb-4 text-sm font-medium rounded-lg animate-fadeIn" role="status" aria-live="polite" aria-atomic="true"
                  :class="{'bg-amber-100 text-amber-800': feedback.type === 'success', 'bg-slate-100 text-slate-700': feedback.type === 'error', 'bg-slate-100 text-slate-700': feedback.type === 'info'}">
                <component :is="feedback.type === 'success' ? PhCheckCircle : (feedback.type === 'info' ? PhLightbulb : PhWarningCircle)" class="w-5 h-5 shrink-0 mt-0.5" weight="fill" />
            ...
            <div class="flex items-center gap-4">
              ...
              <!-- Show 'why' button -->
              <div v-if="showWhy && !showReflection" class="flex-1 flex justify-center">
                <button @click="showReflection = true; showWhy = false"
              ...
              <!-- Continue button -->
              <button v-if="phase === 'done' || reflectionDone"
                      @click="handleNext"
