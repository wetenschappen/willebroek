...
function endDrag() {
  ...
  if (passed) {
    ...
    if (currentInternalLevel.value === 0 && !isObtuse) {
      ...
      if (isRight) {
        errorDetected.value = 'Dit is een rechthoekige driehoek (90°), geen stomphoekige. Maak de hoek groter dan 90° door een punt naar binnen te slepen.'
      } else {
        errorDetected.value = `Alle hoeken zijn kleiner dan 90°. Sleep een hoekpunt naar binnen zodat een hoek > ${currentLevelData.value.threshold}° wordt. Max is nu ${maxA.toFixed(1)}°.`
      }
    } else if (currentInternalLevel.value === 1 && !isRight) {
      if (isObtuse) {
        errorDetected.value = 'Deze hoek is stomp (> 95°). Je moet hem verkleinen tot hij dicht bij 90° is. Sleep het hoekpunt voorzichtig.'
      } else {
        errorDetected.value = `Geen hoek is dicht bij 90°. Je hebt een hoek nodig binnen ${currentLevelData.value.tolerance.toFixed(1)}° van 90°. Max is ${maxA.toFixed(1)}°.`
      }
    } else if (currentInternalLevel.value === 2 && maxA >= currentLevelData.value.maxAngle) {
      if (isRight) {
        errorDetected.value = 'Deze hoek is 90° — dat is een rechte hoek. Voor een scherphoekige driehoek moeten ALLE hoeken kleiner dan 90° zijn.'
      } else if (isObtuse) {
        errorDetected.value = 'De grootste hoek is stomp (> 90°). Maak alle hoeken kleiner dan 90° door de punten te verplaatsen.'
      } else {
        errorDetected.value = `De grootste hoek is ${maxA.toFixed(1)}°, maar moet kleiner dan ${currentLevelData.value.maxAngle}° zijn. Verklein de grootste hoek.`
      }
    } else {
      errorDetected.value = 'Niet de gevraagde driehoek.'
    }
