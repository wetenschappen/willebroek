...
function checkReflection() {
  ...
  const conceptWords = ['nul', 'begin', 'start', 'streepje', '0']
  const reasonWords = ['omdat', 'want', 'doordat', 'anders']
  if ((conceptWords.some(w => ans.includes(w)) || ans.length > 15) && 
    ...
    feedback.value = { type: 'info', text: 'Gebruik conceptwoorden (nul, begin) en leg uit waarom.' }
  }
}
      ...
      <main class="flex flex-1 overflow-hidden">
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div v-if="showWhy && isCorrect && phase === 'measure'" class="mt-4 p-4 border border-amber-200 bg-amber-50 rounded-xl animate-fadeIn">
