...
function onSliderRelease() {
  ...
  if (userAngle.value === 180 - target) {
    errorDetected.value = `Let op de schaal! Je leest de verkeerde boog af. Je hebt nu een hoek van ${userAngle.value}° getekend — dat is ${180 - userAngle.value}° op de andere boog. Gebruik de ${target > 90 ? 'BUITENSTE' : 'BINNENSTE'} boog.`
    feedback.value = {
      type: 'error',
      text: currentLevelData.value.hintLevels[0]
    };
    return;
  }
  ...
  if (attemptCount.value === 1) {
    errorDetected.value = `Je huidige hoek is ${userAngle.value}°.`
    feedback.value = {
      type: 'error',
      text: currentLevelData.value.hintLevels[0]
    };
    ...
    if (target > 90) {
      errorDetected.value = `Je zit er ${diff}° naast. Probeer de BUITENSTE boog af te lezen — begin aan de linkerkant van de geodriehoek.`
    } else {
      errorDetected.value = `Je zit er ${diff}° naast. Gebruik de BINNENSTE boog en tel vanaf 0 naar rechts.`
    }
    feedback.value = { type: 'error', text: currentLevelData.value.hintLevels[1] };
  } else {
    errorDetected.value = `Probeer de slider precies op ${target}° te zetten. Kijk naar de streepjes — elk groot streepje is 10°, elk klein streepje is 1°.`
    feedback.value = { type: 'error', text: currentLevelData.value.hintLevels[2] };
  }
}

function checkReflection() {
  const ans = reflectionAnswer.value.toLowerCase()
  const conceptWords = ['binnenste', 'buitenste', 'scherp', 'stomp', '90']
  const reasonWords = ['omdat', 'want', 'doordat', 'als', 'dan']
  if ((conceptWords.some(w => ans.includes(w)) || ans.length > 15) && 
    ...
    feedback.value = { type: 'info', text: 'Gebruik conceptwoorden (binnenste, buitenste) en leg uit waarom.' }
  }
}
