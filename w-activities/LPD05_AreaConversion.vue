    ...
    if (!isCorrect.value && draggedItem !== null) {
        ...
        if (droppedAns.value === currentLevelData.value.targetVal) {
          ...
          if (droppedAns.value === 10) {
            errorDetected.value = 'Niet helemaal... Let op: 10 is de sprong voor LENGTE (bijv. 1 m = 10 dm). We zoeken OPPERVLAKTE (lengte x breedte). Omdat je de eenheid in twee richtingen verkleint, is de factor 10 x 10 = 100, niet 10.'
            feedback.value = { type: 'error', text: currentLevelData.value.hintLevels[0] }
          } else if (droppedAns.value === 1000) {
            errorDetected.value = '1000 is voor INHOUD (3D): 1 m³ = 1000 dm³. Voor oppervlakte (2D) is de factor 100 per stap, niet 1000.'
            feedback.value = { type: 'error', text: currentLevelData.value.hintLevels[0] }
          } else if (attemptCount.value === 1) {
            errorDetected.value = 'Niet helemaal juist. Gebruik de vergrootglazen in de figuur om het na te tellen.'
            feedback.value = { type: 'error', text: currentLevelData.value.hintLevels[0] }
          } else {
            errorDetected.value = 'Niet het juiste antwoord.'
...
function checkReflection() {
  ...
  const conceptWords = ['lengte', 'breedte', 'twee', 'richtingen', 'oppervlakte']
  const reasonWords = ['omdat', 'want', 'doordat']
  if ((conceptWords.some(w => ans.includes(w)) || ans.length > 15) && 
      (reasonWords.some(w => ans.includes(w)) || ans.length > 25)) {
    reflectionDone.value = true
    showReflection.value = false
    feedback.value = { type: 'success', text: 'Mooie reflectie!' }
  } else {
    feedback.value = { type: 'info', text: 'Gebruik conceptwoorden (lengte, breedte, twee richtingen) en leg uit waarom.' }
  }
}
      ...
      <main class="flex flex-1 overflow-hidden">
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <!-- Why Explanation -->
            <div v-if="phase === 'why' || phase === 'reflect'" class="mt-4 p-4 border border-amber-200 bg-amber-50 rounded-xl animate-fadeIn">
            ...
            <!-- Reflection Phase -->
            <div v-if="showReflection" class="mt-4 p-4 border border-amber-200 bg-amber-50 rounded-xl animate-fadeIn">
              <h4 class="flex items-center gap-2 text-sm font-bold text-amber-800 mb-2">
                Reflectie
              </h4>
              <p class="text-sm text-slate-600 mb-3">Waarom is de factor bij oppervlaktematen altijd een veelvoud van 100 en niet 10?</p>
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <div class="flex items-center gap-4">
              ...
              <!-- Why to reflection -->
              <button v-if="phase === 'why'" @click="proceedAfterWhy"
                      class="flex items-center justify-center flex-1 gap-2 py-4 font-bold text-white transition-all rounded-lg shadow-md bg-amber-600 hover:bg-amber-500 active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:outline-none">
                <span>Naar reflectie</span>
                <PhArrowRight weight="bold" />
              </button>

              <!-- Reflection: submit -->
              <button v-if="showReflection" @click="checkReflection"
                      class="flex items-center justify-center flex-1 gap-2 py-4 font-bold text-white transition-all rounded-lg shadow-md bg-amber-600 hover:bg-amber-500 active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:outline-none"
                      :disabled="reflectionAnswer.trim().length < 3">
                <span>Bevestig reflectie</span>
              </button>
              <button v-if="reflectionDone" @click="handleNext"
                      class="flex items-center justify-center flex-1 gap-2 py-4 font-bold text-white transition-all rounded-lg shadow-md bg-amber-600 hover:bg-amber-500 active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:outline-none">
