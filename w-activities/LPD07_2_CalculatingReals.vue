...
function handleNext() {
  ...
  else if (showReflection.value && allReflectionsDone.value) { checkReflection() }
}
      ...
      <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 md:flex z-10">
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <details class="bg-amber-50 rounded-xl p-4 border border-amber-200 mb-4">
            ...
            <div class="mt-3 text-sm space-y-2">
              <p class="flex items-center gap-2"><PhCheckCircle weight="bold" class="w-4 h-4 text-amber-500 shrink-0" /> √48: zoek delers die een kwadraat zijn.</p>
              <p class="flex items-center gap-2"><PhCheckCircle weight="bold" class="w-4 h-4 text-amber-500 shrink-0" /> 48 = 16 × 3. 16 is een kwadraat (4^2).</p>
              <p class="flex items-center gap-2"><PhCheckCircle weight="bold" class="w-4 h-4 text-amber-500 shrink-0" /> √48 = √(16 × 3) = √16 × √3 = 4√3.</p>
              <p class="flex items-center gap-2"><PhCheckCircle weight="bold" class="w-4 h-4 text-amber-500 shrink-0" /> Klaar! 4√3 is de vereenvoudigde vorm.</p>
            </div>
          </details>
          ...
          <div v-if="!predictionMade" class="bg-amber-50 rounded-xl p-4 border border-amber-200 mb-4">
            ...
            <p class="text-xs text-amber-600 mb-3">Hoe ga je de wortel vereenvoudigen?</p>
            <div class="space-y-2">
              <label v-for="opt in predictionOptions" :key="opt.id" class="flex items-center gap-2 p-2 rounded-lg bg-white border cursor-pointer text-xs"
                ...
                <input type="radio" :value="opt.id" v-model="predictionChoice" class="accent-amber-500" /> {{ opt.text }}
              </label>
            </div>
          ...
          <div v-if="predictionMade" class="space-y-4">
            ...
            <div class="p-4 border border-amber-200 bg-amber-50 rounded-xl">
              <div class="flex items-center justify-between gap-2">
                ...
                <div class="flex-1">
                  ...
                  <input type="number" v-model.number="inputRemainder" :disabled="step > 0" class="w-full p-4 text-xl font-mono font-black text-center text-amber-600 bg-white border-2 border-amber-300 rounded-lg" placeholder="?">
                </div>
              </div>
            </div>
            <div v-if="showWhy" class="bg-amber-50 rounded-xl p-4 border border-amber-200 animate-fadeIn">
              ...
              <p v-for="(l, i) in whyText" :key="i" class="text-xs text-amber-700 mb-1">{{ l }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="p-4 bg-slate-50 border-t border-slate-200 shrink-0">
          <div v-if="feedback.text" class="flex items-start gap-3 p-3 mb-3 text-sm rounded-lg animate-fadeIn"
            :class="{'bg-amber-50': feedback.type === 'success', 'bg-red-100': feedback.type === 'error', 'bg-blue-100': feedback.type === 'info'}">
          ...
          <div v-else-if="!showReflection && !showLPD" class="flex items-center gap-3">
            ...
            <button v-else @click="handleNext" :disabled="step < 2" class="flex-1 py-3 font-bold text-white rounded-lg bg-amber-600 hover:bg-amber-500 disabled:opacity-50">Reflecteer <PhArrowRight weight="bold" class="inline" /></button>
          </div>
          ...
          <div v-if="showReflection" class="space-y-3">
            ...
            <div v-for="(q, i) in reflectionQuestions" :key="i" class="space-y-1">
              ...
              <textarea v-model="reflectionAnswers[i]" rows="2" class="w-full text-sm p-2 border border-amber-200 rounded-lg bg-white resize-none" placeholder="Min 10 tekens..."></textarea>
            </div>
            <button @click="checkReflection" :disabled="!allReflectionsDone" class="w-full py-2 bg-indigo-600 text-white font-bold rounded-lg disabled:opacity-50 text-sm">
        ...
        <div class="flex flex-1 p-6 items-center justify-center relative pattern-grid">
          <div class="w-full max-w-4xl flex items-center justify-center gap-4">
            ...
            <div class="flex items-center gap-4" :class="step > 0 ? 'opacity-100' : 'opacity-0'">
              ...
              <div class="text-6xl font-mono font-black text-amber-600" :class="step === 2 ? '-translate-x-8 text-7xl' : ''">√<span class="border-t-[6px] border-emerald-600 pt-2 ml-1">{{ inputRemainder || '?' }}</span></div>
            </div>
          </div>
        </div>
      </div>
