    ...
    if (allCorrect) {
      ...
      if (data.errorPatterns) {
        for (const pattern of data.errorPatterns) {
          const matchingNum = required.find(n => placements.value[n] === pattern.zone && pattern.check(n))
          if (matchingNum) {
            errorDetected.value = pattern.message
            break
          }
        }
      }
...
function checkReflection() {
  ...
  const conceptWords = ['omdat', 'want', 'bevat', 'n', 'z', 'q']
  if (ans.length > 10) {
