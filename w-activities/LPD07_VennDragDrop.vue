...
function autoCheck() {
  ...
  if (allPlaced) {
    ...
    if (allCorrect) {
      ...
      errorDetected.value = ""
      if (wrongItem) {
        if (wrongItem.targetRegion === 'intersection') {
          errorDetected.value = `${wrongItem.value} hoort in de doorsnede (A ∩ B), want het heeft beide eigenschappen.`
        } else if (wrongItem.targetRegion === 'outside') {
          errorDetected.value = `${wrongItem.value} heeft geen van beide eigenschappen en hoort dus erbuiten.`
        } else if (wrongItem.targetRegion === 'aOnly') {
          errorDetected.value = `${wrongItem.value} hoort alleen in ${data.labelA}.`
        } else if (wrongItem.targetRegion === 'bOnly') {
          errorDetected.value = `${wrongItem.value} hoort alleen in ${data.labelB}.`
        }
      }
...
function checkReflection() {
  ...
  const conceptWords = ["venn", "verzameling", "doorsnede", "vereniging", "even", "priem", "veelvoud", "deelverzameling", "getal"]
  const reasonWords = ["omdat", "dus", "daarom", "want", "als", "dan", "doordat", "alleen", "beide"]
      ...
      <main class="flex flex-1 overflow-hidden">
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <!-- Error Block (Let op!) -->
            <div v-if="!isCorrect && attemptCount > 0 && errorDetected" class="flex items-start gap-3 p-3 mb-3 text-sm font-medium rounded-lg bg-red-50 text-red-800 border border-red-200 animate-fadeIn">
