...
function checkPrediction() {
  ...
  if (predictedAnswer.value === prob.correct) {
    ...
    attemptCount.value++
    errorDetected.value = `Tip: ${prob.hint}`
    feedback.value = { type: 'error', text: `Niet correct.` }
  }
}
...
watch(tiltValue, (val) => {
  ...
  if (val === correctTilt) {
    ...
    errorDetected.value = ""
    showWhy.value = true
    whyText.value = prob.leftVal < 0 || prob.rightVal < 0 ? 'Op de getallenas liggen negatieve getallen links van 0. Hoe kleiner het getal, hoe verder het van 0 verwijderd is. -0,5 is kleiner dan -0,3 omdat -0,5 links van -0,3 ligt.' : 'Door breuken gelijknamig te maken (zelfde noemer), kun je de tellers rechtstreeks vergelijken. 1/3 en 1/4 worden 4/12 en 3/12 — en 4 > 3.'
    reflectionAnswer.value = ''
    feedback.value = { type: 'success', text: `Correct! ${prob.left} ${currentSymbol.value} ${prob.right}` }
  } else if (val !== 0) {
    attemptCount.value++
    errorDetected.value = `Dit weegschaal evenwicht klopt niet. Controleer opnieuw de breuken of decimalen.`
  }
})
...
function triggerHint() {
  ...
  hintCount.value++
  feedback.value = { type: 'info', text: currentLevelData.value.hint }
}
      ...
      <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <div v-if="showReflection" class="mt-4 p-4 bg-amber-50 rounded-lg border-l-4 border-amber-400 animate-fadeIn">
            <p class="font-bold text-amber-800 mb-2 text-sm">{{ currentLevelData.leftVal < 0 || currentLevelData.rightVal < 0 ? 'Waarom is -0,5 < -0,3 terwijl 0,5 > 0,3?' : 'Hoe heb je de getallen vergeleken?' }}</p>
        ...
        <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
          <div v-if="feedback.text" class="flex items-start gap-3 p-3 mb-4 text-sm font-medium rounded-lg animate-fadeIn border-l-4"
            :class="{ 'bg-amber-50 text-amber-800 border-amber-400': feedback.type === 'success', 'bg-slate-100 text-slate-700 border-slate-400': feedback.type === 'error', 'bg-slate-100 text-slate-700 border-slate-400': feedback.type === 'info' }">
          ...
          <div class="flex items-center gap-4">
            ...
            <button v-if="isCorrect && !showReflection" @click="handleNext" class="flex-1 py-4 font-bold text-white rounded-lg shadow-md bg-amber-600 hover:bg-amber-500 active:scale-[0.98] flex items-center justify-center gap-2">
