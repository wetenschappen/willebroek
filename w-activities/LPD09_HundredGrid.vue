...
function checkReflection() {
  ...
  const conceptWords = ["procent", "honderd", "percentage", "breuk", "verhouding", "deel", "geheel", "grid", "rooster", "hokje"]
  const reasonWords = ["omdat", "dus", "daarom", "want", "als", "dan", "doordat", "betekent", "stel"]
  ...
  if (hasConcept && hasReason && minLen) {
    reflectionSubmitted.value = true; showLpdConfirm.value = true
    return true
  }
  if (!minLen) feedback.value = { type: 'error', text: 'Schrijf een volledige zin (minimaal 20 tekens).' }
  else if (!hasConcept) feedback.value = { type: 'error', text: 'Probeer vaktaal te gebruiken bij je antwoord.' }
      ...
      <main class="flex flex-1 overflow-hidden">
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div v-if="showWorkedExample && predictionSubmitted" class="animate-fadeIn">
              <div class="p-4 bg-amber-50 border border-amber-200 rounded-xl shadow-sm mb-4">
                <h4 class="font-bold text-amber-800 text-sm uppercase tracking-wider mb-3 flex items-center gap-2">
                  <PhLightbulb weight="fill" class="w-4 h-4" />
                  Voorbeeld: 25% op het honderdveld
                </h4>
                <div class="space-y-2 mb-3">
                  <div class="flex items-start gap-2">
                    <span class="w-6 h-6 rounded-full bg-amber-200 text-amber-700 text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">1</span>
                    <span class="text-sm text-slate-700">25% betekent 25 per honderd. Op een honderdveld = 25 hokjes.</span>
                  </div>
                  <div class="flex items-start gap-2">
                    <span class="w-6 h-6 rounded-full bg-amber-200 text-amber-700 text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">2</span>
                    <span class="text-sm text-slate-700">25 hokjes = 25/100 = 0,25 = 25%. Kleur 5 rijen van 5.</span>
                  </div>
                  <div class="flex items-start gap-2">
                    <span class="w-6 h-6 rounded-full bg-amber-200 text-amber-700 text-xs font-bold flex items-center justify-center shrink-0 mt-0.5">3</span>
                    <span class="text-sm text-slate-700">Vul daarna het kommagetal (0,25) en percentage (25%) in.</span>
                  </div>
                </div>
                <div class="p-2 bg-amber-100 rounded-lg text-center font-bold text-amber-800 font-mono text-sm">
                  25% = 25/100 = 0,25
                </div>
              </div>
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <!-- REFLECTION -->
            <div v-if="showReflection" class="p-3 mb-3 bg-amber-50 border border-amber-200 rounded-lg animate-fadeIn">
            ...
            <!-- LPD CONFIRMATION -->
            <div v-if="showLpdConfirm && !lpdConfirmed" class="p-3 mb-3 bg-amber-50 border border-amber-200 rounded-lg animate-fadeIn">
              <p class="text-xs font-bold text-amber-800 uppercase tracking-wider mb-2 flex items-center gap-1">
              ...
              <button @click="confirmLpd"
                      class="w-full py-2 text-sm font-bold bg-amber-600 text-white rounded-lg hover:bg-amber-500 transition-colors focus-visible:ring-2 focus-visible:ring-amber-500">
              ...
              <div class="relative bg-white p-2 rounded-xl shadow-md border-2 border-slate-300"
                ...
                <div class="grid grid-cols-10 grid-rows-10 gap-1">
                  <div v-for="(block, index) in grid" :key="index"
                       ...
                       :class="block ? (isCorrect ? 'bg-amber-500 border-emerald-600 shadow-inner' : 'bg-amber-500 border-amber-600 shadow-inner') : 'bg-slate-100 border-slate-300 hover:bg-slate-200'">
                  </div>
                </div>
