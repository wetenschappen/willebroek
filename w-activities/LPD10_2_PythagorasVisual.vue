        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div v-if="showWorkedExample && currentInternalLevel === 0"
                 class="mb-4 border border-amber-200 bg-amber-50 rounded-xl p-4 animate-fadeIn">
              <div class="flex items-center gap-2 mb-3">
                <PhBookOpen class="w-5 h-5 text-amber-600" weight="fill" />
                <h4 class="font-bold text-amber-800">Voorbeeld: Pythagoras visueel</h4>
              </div>
              <div class="space-y-1">
                <div v-for="(s, i) in workedSteps" :key="i"
                     @click="workedStep = i"
                     class="flex items-center gap-3 p-2 rounded-lg cursor-pointer"
                     :class="workedStep === i ? 'bg-amber-100' : 'hover:bg-amber-100/50'">
                  <div class="w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold shrink-0"
                       :class="workedStep >= i ? 'bg-amber-500 text-white' : 'bg-amber-200 text-amber-400'">
                    {{ i + 1 }}
                  </div>
                  <div>
                    <p class="text-sm font-medium"
                       :class="workedStep === i ? 'text-amber-900' : 'text-amber-600'">
                      {{ s.label }}
                    </p>
                    <p v-if="workedStep === i" class="text-xs text-amber-500">
                      {{ s.detail }}
                    </p>
                  </div>
                </div>
              ...
              <button @click="showWorkedExample = false"
                      class="mt-3 w-full py-2 text-sm font-bold text-amber-600 bg-white rounded-lg border border-amber-200 hover:bg-amber-50 transition-colors">
            ...
            <!-- WHY EXPLANATION -->
            <div v-if="showWhy"
                 class="p-4 mb-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
              ...
              <MathText :content="currentLevelData.whyText"
                        class="text-sm text-amber-700 leading-relaxed" />
            </div>
            ...
            <div v-if="showLPDEvidence && !lpdDone"
                 class="mt-6 p-5 border-2 border-amber-300 bg-amber-50 rounded-xl animate-fadeIn">
              <div class="flex items-center gap-2 mb-3">
                <PhTarget class="w-5 h-5 text-amber-600" weight="fill" />
                <span class="font-bold text-amber-800">LPD-bewijs</span>
              </div>
              <p class="text-sm text-amber-700 mb-3">
                {{ currentLevelData.lpdText }}
              </p>
              <button @click="handleLPDDone"
                      class="px-4 py-2 text-sm font-bold text-white bg-amber-600 rounded-lg hover:bg-amber-500 transition-all focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:outline-none">
          ...
          <div class="flex flex-col flex-1 p-6 overflow-y-auto">
            ...
            <div class="relative flex-1 flex items-center justify-center w-full min-h-[400px] p-8 bg-slate-100 rounded-xl border-2 border-slate-200/50 pattern-grid overflow-hidden">
              ...
              <div v-if="currentLevelData"
                ...
                <div class="absolute font-bold text-amber-800 bg-white/80 px-1 rounded-lg tabular-nums transform -rotate-[53deg]"
                ...
                <div class="absolute flex flex-col items-center justify-center bg-blue-100 border-4 border-blue-400 rounded-lg shadow-inner"
                  ...
                  <div v-else
                       ...
                       :class="slotA ? (isCorrect ? 'border-emerald-500 bg-amber-50 text-amber-600' : 'border-blue-500 text-blue-600') : 'border-blue-300'"
                       @click="removeSlot('A')"
                ...
                <div class="absolute flex flex-col items-center justify-center bg-amber-100 border-4 border-amber-400 rounded-lg shadow-inner"
                  ...
                  <div v-else
                       ...
                       class="w-14 h-14 border-4 border-dashed rounded-xl flex items-center justify-center bg-white shadow-sm transition-colors cursor-pointer focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:outline-none min-w-[44px] min-h-[44px]"
                       :class="slotB ? (isCorrect ? 'border-emerald-500 bg-amber-50 text-amber-600' : 'border-amber-500 text-amber-600') : 'border-amber-300'"
                       @click="removeSlot('B')"
                       role="button" tabindex="0"
                ...
                <!-- Square C (Hypotenuse) -->
                <div class="absolute flex flex-col items-center justify-center bg-amber-50 border-4 border-amber-400 rounded-lg shadow-inner"
                  ...
                  <div class="transform -rotate-[36.87deg] flex flex-col items-center">
                    <span class="text-amber-600 font-bold text-xs mb-2">c\u00b2</span>
                    <div v-if="currentLevelData.areaC_given !== null"
                         class="text-4xl font-mono font-black text-amber-600 tabular-nums">
                      {{ slotC }}
                    </div>
                    <div v-else
                         @dragover.prevent.stop
                         ...
                         :class="slotC ? (isCorrect ? 'border-emerald-500 bg-amber-50 text-amber-600' : 'border-emerald-500 text-amber-600') : 'border-amber-300'"
                         @click="removeSlot('C')"
                         role="button" tabindex="0"
            ...
            <div v-if="isCorrect && currentLevelData"
                 class="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-xl text-center animate-fadeIn">
              <p class="font-bold text-amber-800 text-lg font-mono tabular-nums">
