...
function submitPrediction() {
  ...
  if (predictedGCD.value === d.targetAns) {
    ...
    if (predictedGCD.value > d.targetAns) {
      if (d.targetAns === 1) {
        errorDetected.value = `${predictedGCD.value} is te groot. ${d.wallW} en ${d.wallH} hebben geen gemeenschappelijke deler groter dan 1 — ze zijn 'relatief priem'. De GGD is dan altijd 1.`
      } else {
        errorDetected.value = `${predictedGCD.value} is te groot. Dit getal deelt niet beide getallen. De GGD moet beide getallen delen zonder rest.`
      }
    } else if (predictedGCD.value > 0 && d.targetAns % predictedGCD.value === 0) {
      errorDetected.value = `Goed opgemerkt: ${predictedGCD.value} is wel een deler van beide, maar het is niet de GROOTSTE. Er is een grotere gemeenschappelijke deler.`
    } else if (predictedGCD.value === 0) {
      errorDetected.value = 'GGD is nooit 0 (behalve voor 0 zelf). Elke deler is minstens 1. Wat is een getal dat beide deelt zonder rest?'
    } else {
      errorDetected.value = `Let op: ${predictedGCD.value} deelt niet beide getallen. GGD is de GROOTSTE GEMEENSCHAPPELIJKE deler. Soms denken leerlingen aan KGV (kleinste gemeenschappelijk veelvoud) — GGD en KGV worden vaak verward! GGD = grootste deler die beide deelt.`
    }
    feedback.value = { type: 'error', text: 'Je voorspelling is niet correct, probeer opnieuw.' }
  }
}
