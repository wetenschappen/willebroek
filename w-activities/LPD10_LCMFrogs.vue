...
function submitPrediction() {
  ...
  if (predictedKGV.value === d.targetAns) {
    ...
    if (predictedKGV.value > 0 && d.jump1 % predictedKGV.value === 0 && d.jump2 % predictedKGV.value === 0) {
      errorDetected.value = `Let op: ${predictedKGV.value} is een deler van beide, dus dat is een GGD (Grootste Gemeenschappelijke Deler). Maar we zoeken het KGV (Kleinste Gemeenschappelijk Veelvoud). GGD en KGV worden vaak verward! Denk aan veelvouden (tafels van beide getallen), niet aan delers.`
    } else if (predictedKGV.value === d.jump1 * d.jump2 && d.targetAns !== d.jump1 * d.jump2) {
      errorDetected.value = `KGV is niet altijd het product van de twee getallen. Kijk naar de tafels van ${d.jump1} en ${d.jump2} — het eerste gemeenschappelijke getal is kleiner dan ${d.jump1 * d.jump2}.`
    } else if (predictedKGV.value >= d.targetAns && predictedKGV.value % d.jump1 === 0 && predictedKGV.value % d.jump2 === 0) {
      errorDetected.value = `${predictedKGV.value} is een veelvoud van beide, maar niet het KLEINSTE. Er is een kleiner getal waar ze elkaar ontmoeten.`
    } else if (predictedKGV.value > 0 && predictedKGV.value < d.targetAns) {
      errorDetected.value = `${predictedKGV.value} is te klein. Dit getal is niet door beide tegelijk deelbaar. Laat de kikkers springen om te zien waar ze stoppen.`
    } else {
      errorDetected.value = `${predictedKGV.value} klopt niet. Denk aan de tafels van ${d.jump1} en ${d.jump2}. Het eerste gemeenschappelijke getal is het KGV.`
    }
    feedback.value = { type: 'error', text: 'Voorspelling niet correct. Probeer opnieuw.' }
  }
}
...
function checkAnswer() {
  ...
  if (meetPoint.value === d.targetAns) {
    ...
    if (meetPoint.value && meetPoint.value !== d.targetAns) {
      errorDetected.value = `De kikkers ontmoeten elkaar nu op ${meetPoint.value}, maar dat is niet de KGV. Spring verder!`
    } else {
      errorDetected.value = 'Laat de kikkers verder springen tot ze elkaar ontmoeten.'
    }
    feedback.value = { type: 'error', text: 'Nog niet klaar. Probeer nog eens.' }
  }
}
        ...
        <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
          <div
            ...
            :class="{
              ...
              'bg-slate-100 text-slate-700 border-slate-400': feedback.type === 'error',
          ...
          <!-- Error Block (Let op!) -->
          <div v-if="!isCorrect && attemptCount > 0 && errorDetected" class="flex items-start gap-3 p-3 mb-4 text-sm font-medium rounded-lg bg-red-50 text-red-800 border-l-4 border-red-400 animate-fadeIn">
