...
function showHint() {
  ...
  const data = currentLevelData.value
  const hints = [
    `Bij een macht van een macht: (${data.base}^${data.innerExp})^${data.outerExp} betekent dat we ${data.base}^${data.innerExp} ${data.outerExp} keer herhalen.`,
    `Voorbeeld: (a³)² = a³ × a³ = a⁶. We vermenigvuldigen de exponenten: 3 × 2 = 6.`,
    `Methode: (${data.base}^${data.innerExp})^${data.outerExp} = ${data.base}^(${data.innerExp} × ${data.outerExp}) = ${data.base}^___`
  ]
...
function checkAnswer() {
  ...
  if (userAns.value === data.exactAns) {
    if (step.value === 2) {
        ...
        const whyExplanation = `Je ziet letterlijk ${data.outerExp} groepjes van ${data.innerExp}. Dat is ${data.outerExp} × ${data.innerExp} = ${data.exactAns} ${data.base}'tjes. (${data.base}^${data.innerExp})^${data.outerExp} betekent dat we ${data.base}^${data.innerExp} nemen en dit ${data.outerExp} keer met zichzelf vermenigvuldigen. Elke keer herhalen we de ${data.innerExp} factoren. Dus in totaal hebben we ${data.outerExp} groepen van elk ${data.innerExp} factoren — dat is ${data.outerExp} × ${data.innerExp} = ${data.exactAns} keer de basis. Daarom vermenigvuldig je de exponenten, je telt niet op.`;
        showWhyExplanation(whyExplanation);
        
        const lpdText = currentInternalLevel.value === 0
          ? 'LPD 15: Je past de regel (aᵐ)ⁿ = aᵐⁿⁿ correct toe.'
          : currentInternalLevel.value === 1
            ? 'LPD 15: Je analyseert waarom de exponenten vermenigvuldigd worden.'
            : 'LPD 15: Je evalueert complexe machten en begrijpt de onderliggende structuur.';
        feedback.value = { type: 'success', text: lpdText };
        showReflection.value = true;
    } else {
        isCorrect.value = false
        errorDetected.value = "Je antwoord klopt wel, maar je sloeg de tussenstappen over."
    ...
    if (userAns.value === data.innerExp + data.outerExp) {
        errorDetected.value = `Je hebt de exponenten opgeteld (${data.innerExp} + ${data.outerExp} = ${userAns.value}). We herhalen een volledige groep!`
        feedback.value = { type: 'error', text: `Let op: bij een macht van een macht (${data.base}^${data.innerExp})^${data.outerExp} herhalen we de hele groep ${data.outerExp} keer. Elk groepje bevat ${data.innerExp} factoren. In totaal zijn dat ${data.outerExp} groepen van ${data.innerExp} — dat is een vermenigvuldiging, geen optelling.<br><br>Klik op "Kopieer" en daarna "Vouw Open" om het visueel te zien.` }
    } else if (userAns.value === Math.pow(data.innerExp, data.outerExp)) {
        errorDetected.value = `Je hebt ${data.innerExp}^${data.outerExp} berekend. Dat is een macht OVER de exponent, dat is hier niet van toepassing.`
        feedback.value = { type: 'error', text: `De rekenregel werkt anders. Volg de visuele blokjes.` }
    } else if (attemptCount.value >= 3) {
        errorDetected.value = "Niet het juiste totaal aantal."
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div v-if="showReflection && !reflectDone" class="p-5 border-2 border-amber-300 bg-amber-50 rounded-xl mb-4 animate-fadeIn">
              ...
              <button @click="checkReflection" :disabled="reflectAns.trim().length === 0"
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            <div v-if="feedback.text" class="flex items-start gap-4 p-4 mb-4 text-sm font-medium rounded-lg animate-fadeIn" role='status' aria-live='polite' aria-atomic='true' :class="{'bg-amber-100 text-amber-800': feedback.type === 'success', 'bg-slate-100 text-slate-700': feedback.type === 'error', 'bg-amber-50 text-amber-700': feedback.type === 'info'}">
