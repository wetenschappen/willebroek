...
watch(userAngle, (newAngle) => {
  ...
  if (currentLevelData.value.validate(newAngle)) {
    ...
    if (attemptCount.value >= 5) {
      ...
      errorDetected.value = `Niet juist. ${hints[hintIdx] || 'Probeer een andere hoek.'}`
      feedback.value = {
        type: 'error',
        text: `Blijf de slider aanpassen tot je de juiste hoek vindt.`
      }
    }
  }
})
      ...
      <main class="flex flex-1 overflow-hidden">
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <!-- Error Block (Let op!) -->
            <div v-if="!isCorrect && attemptCount >= 5 && errorDetected" class="flex items-start gap-3 p-4 mb-4 text-sm font-medium rounded-lg bg-red-50 text-red-800 border-l-4 border-red-400 animate-fadeIn">
