...
function checkPrediction() {
  ...
  if (predictedParentOp.value === corr) {
    ...
    errorDetected.value = 'Dat is niet de juiste bewerking om mee te beginnen.'
  ...
  if (isNum && Number(userAnswer.value) === Number(currentLevelData.value.answer)) {
    ...
    if (currentInternalLevel.value === 0) {
      ...
      if (userNum === (currentLevelData.value.answer * 2)) {
        errorDetected.value = 'Je hebt de macht overslagen. Typische fout! Herinner: machten gaan ALTIJD voor ×, ÷, + en -.'
      } else if (attemptCount.value >= 3) {
        errorDetected.value = 'Blijf de formule stap voor stap ontleden. Welke operator heeft de hoogste prioriteit? Werk van hoog naar laag.'
      } else {
        errorDetected.value = 'Niet correct. De bewerkingen moeten in een specifieke volgorde gebeuren.'
      }
    } else if (currentInternalLevel.value === 1) {
      if (userNum === (currentLevelData.value.answer * 2)) {
        errorDetected.value = 'Je hebt de haakjes overgeslagen. Herinner: haakjes ( ) hebben absolute voorrang, nog vóór machten!'
      } else {
        errorDetected.value = 'Niet correct. Werk stap voor stap: eerst de haakjes, dan ×/÷ (L→R), dan +/- (L→R).'
      }
    } else {
      if (attemptCount.value >= 2) {
        errorDetected.value = 'Let op: ÷ en × zijn even sterk. Je moet van LINKS naar RECHTS werken, niet vermenigvuldigen voor delen.'
      } else {
        errorDetected.value = 'Niet correct. Herinner: ÷ en × hebben dezelfde prioriteit. Wat betekent dat voor de volgorde?'
      }
    }
    ...
    feedback.value = { type: 'error', text: attemptCount.value >= 3
      ...
      : 'Gebruik de hintknop voor een extra tip.' }
  }
}
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            <div v-if="feedback.text" class="flex items-start gap-3 p-4 mb-4 text-sm font-medium rounded-lg animate-fadeIn"
                 ...
                 :class="{'bg-amber-50 text-amber-800 border border-amber-200': feedback.type === 'success',
                          'bg-slate-50 text-slate-700 border border-slate-200': feedback.type === 'error',
