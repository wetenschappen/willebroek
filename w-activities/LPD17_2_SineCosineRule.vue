      ...
      <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-amber-50 border border-amber-200 rounded-xl p-6 mb-6">
          ...
          <div class="space-y-4 text-slate-700">
            ...
            <div class="p-3 bg-white rounded-lg border border-amber-100">
              <p class="font-bold text-amber-700">Sinusregel (H-H-Z of Z-H-H):</p>
    ...
    <div v-if="showPredictionGate&&!predictionMade" class="absolute inset-0 z-20 bg-white overflow-y-auto p-8">
      <div class="max-w-xl mx-auto text-center">
        ...
        <div class="flex flex-col gap-3">
          <button @click="handlePrediction('cosinus')" class="p-4 border-2 border-amber-200 rounded-xl hover:bg-amber-50 hover:border-amber-400 font-bold text-slate-700 transition-all">Cosinusregel (Z-H-Z)</button>
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div class="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-4">
              ...
              <div v-if="showAmbiguousInput && currentLevel.isAmbiguous">
                <label class="block mb-2 text-sm font-bold text-amber-700">Tweede mogelijke hoek γ₂:</label>
                <input type="number" step="any" v-model="ambiguousAnswer2" class="w-full p-4 text-lg font-bold text-slate-800 bg-white border-2 border-amber-300 rounded-lg outline-none focus:border-indigo-500 transition-colors" placeholder="Bv. 149,0" />
              </div>
            </div>

            <div v-if="showWhySection" class="p-4 mt-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
              <h4 class="font-bold text-amber-800 mb-2 text-sm">Waarom werkt dit?</h4>
              <p class="text-xs text-amber-700">De sinus- en cosinusregel zijn afgeleid uit de eigenschappen van willekeurige (niet-rechthoekige) driehoeken. De cosinusregel is een veralgemening van Pythagoras: als de hoek 90° is, valt de term -2bc·cos(α) weg en krijg je a² = b² + c². De sinusregel volgt uit de hoogtelijnen van een driehoek.</p>
            </div>
            ...
            <div v-if="showReflection&&!reflectionDone" class="p-4 mt-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
              <h4 class="font-bold text-amber-800 mb-3 text-sm">Reflectie</h4>
              <p class="text-xs text-amber-700 mb-3"><strong>Waarom</strong> kan de sinusregel bij Z-Z-H twee oplossingen geven terwijl de cosinusregel bij Z-H-Z altijd een uniek antwoord geeft?</p>
              <button @click="checkReflection" class="px-4 py-2 bg-indigo-600 text-white text-sm font-bold rounded-lg hover:bg-amber-500 transition-colors">Ik heb nagedacht, ga verder</button>
            </div>

            <div v-if="showLPDEvidence&&!lpdDone" class="p-4 mt-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
              <h4 class="font-bold text-amber-800 mb-3 text-sm">LPD 17: Bewijs van beheersing</h4>
              <p class="text-xs text-amber-700 mb-3">Je hebt bewezen dat je sinus- en cosinusregel kan toepassen. Leg uit: "Ik gebruik de _____ regel als ik twee zijden en de ingesloten hoek ken. Ik gebruik de _____ regel als ik een overstaand paar (zijde+hoek) en nog een waarde ken."</p>
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <div class="flex items-center gap-4">
              ...
              <button v-else @click="handleNext" class="flex items-center justify-center flex-1 gap-2 py-4 font-bold text-white transition-all rounded-lg shadow-md bg-amber-600 hover:bg-amber-500 active:scale-[0.98]"><span>{{ currentInternalLevel<totalInternalLevels-1?'Volgend Level':'Afronden' }}</span><PhArrowRight weight="bold" /></button>
            </div>
          </div>
        </div>
            ...
            <div class="bg-white p-8 rounded-xl shadow-md border-2 border-slate-200 text-center max-w-md">
              ...
              <div class="grid grid-cols-2 gap-4 text-sm">
                ...
                <div class="p-4 rounded-lg bg-amber-50 border border-amber-200">
                  <p class="font-bold text-amber-700 mb-2">Sinusregel</p>
                  <p class="text-amber-600 text-xs">a/sin(α) = b/sin(β)</p>
                  <div class="mt-2 text-xs text-amber-600"><span class="font-bold">H-H-Z</span> of <span class="font-bold">Z-H-H</span></div>
                </div>
              </div>
