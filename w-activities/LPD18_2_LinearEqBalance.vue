...
function checkPrediction() {
  ...
  if (predictedFirstStep.value === 'subConst') {
    ...
    if (predictedFirstStep.value === 'subX') {
      errorDetected.value = `Let op! Je probeert x weg te werken, maar x staat aan beide kanten. Eerst de constante term (-${Math.abs(lvl.targetC || 0)}) aan beide kanten wegwerken.`
      feedback.value = { type: 'error', text: `Volgorde: eerst constanten, dan x.` }
    } else if (predictedFirstStep.value === 'divBoth') {
      errorDetected.value = `Let op! Je wil al delen, maar de vergelijking is nog niet vereenvoudigd. Werk eerst de constante term weg door ${lvl.hints[0]}. Daarna pas delen.`
      feedback.value = { type: 'error', text: `Deel pas aan het einde.` }
    } else {
      errorDetected.value = `Let op! Denk na: wat staat er nog bij x? Eerst dat wegwerken.`
      feedback.value = { type: 'error', text: `Hint: ${lvl.hints[0]}` }
    }
  }
}
...
function applyOperation() {
  ...
  if (opType.value === '/' && val === 0) {
    errorDetected.value = "Delen door 0 is niet toegestaan!"
    feedback.value = { type: 'error', text: 'Kies een ander getal.' }
    return
  }
...
function checkWin() {
  ...
  if (isXOnLeft || isXOnRight) {
    ...
    showWhy.value = true
    whyText.value = "Elke bewerking die je op BEIDE kanten toepast, behoudt het evenwicht van de vergelijking. Dit is de balansmethode: het =-teken garandeert dat linker- en rechterlid gelijk zijn; wat je met het ene lid doet, moet je met het andere doen om de gelijkheid te bewaren."
    ...
    attemptCount.value++
    errorDetected.value = 'Je hebt alle x-en weggewerkt! Reset en probeer opnieuw, maar nu met een andere volgorde.'
    feedback.value = { type: 'error', text: 'Dit is niet meer oplosbaar.' }
  } else {
    attemptCount.value++
    errorDetected.value = "De vergelijking is nog niet opgelost."
    feedback.value = { type: 'info', text: "Kies de volgende bewerking en voer uit op beide kanten." }
  }
}
...
function triggerHint() {
  ...
  hintCount.value++
  feedback.value = { type: 'info', text: currentLevelData.value.hints[hintLevel.value] }
}
      ...
      <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <div v-if="showWhy && !showReflection" class="mt-4 p-4 bg-amber-50 rounded-lg border-l-4 border-amber-400 animate-fadeIn">
            <p class="font-bold text-amber-800 text-sm mb-1">Waarom werkt dit?</p>
            <p class="text-xs text-amber-700 leading-relaxed">{{ whyText }}</p>
          ...
          <div v-if="showReflection" class="mt-4 p-4 bg-amber-50 rounded-lg border-l-4 border-amber-400 animate-fadeIn">
            ...
            <p class="text-xs text-amber-700 mb-2">Waarom moet je altijd aan BEIDE kanten dezelfde bewerking doen? Leg uit in je eigen woorden.</p>
        ...
        <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
          <div v-if="feedback.text" class="flex items-start gap-3 p-3 mb-4 text-sm font-medium rounded-lg animate-fadeIn border-l-4" role="status" aria-live="polite" aria-atomic="true"
            :class="{ 'bg-amber-50 text-amber-800 border-amber-400': feedback.type === 'success', 'bg-red-100 text-red-800 border-red-400': feedback.type === 'error', 'bg-blue-100 text-blue-800 border-blue-400': feedback.type === 'info' }">
          ...
          <div class="flex items-center">
            ...
            <button v-if="!isCorrect" @click="triggerHint" 
                    class="px-4 py-4 font-bold text-slate-600 rounded-lg border-2 border-slate-300 bg-white hover:bg-slate-50 flex items-center gap-2 active:scale-[0.98] mr-4">
              <PhLightbulb weight="fill" class="w-5 h-5 text-amber-500" />
              <span>{{ showHint ? 'Hint ' + (hintLevel + 1) : 'Hint nodig?' }}</span>
            </button>
            <button v-if="isCorrect && !showReflection" @click="handleNext" class="py-4 px-6 font-bold text-white rounded-lg shadow-md bg-amber-600 hover:bg-amber-500 active:scale-[0.98]">
