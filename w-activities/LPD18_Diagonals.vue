    ...
    <main class="flex flex-1 overflow-hidden">
      <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <!-- Why Explanation -->
          <div v-if="showWhy && isCorrect" class="mt-4 p-4 bg-amber-50 rounded-lg border-l-4 border-amber-400 animate-fadeIn">
          ...
          <div v-if="showReflection" class="mt-4 p-4 bg-amber-50 rounded-lg border-l-4 border-amber-400 animate-fadeIn">
            ...
            <p class="text-xs text-amber-700 mb-2">Welke vierhoeken hebben even lange diagonalen? En waarom?</p>
        ...
        <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
          ...
          <button v-if="!isCorrect" @click="showHint ? (hintLevel < 2 ? hintLevel++ : null) : (showHint = true)" class="flex-1 py-4 font-bold text-slate-600 rounded-lg border-2 border-slate-300 bg-white hover:bg-slate-50 transition-colors active:scale-[0.98] focus-visible:ring-2 focus-visible:ring-amber-500 focus-visible:outline-none flex items-center justify-center gap-2 mx-2">
