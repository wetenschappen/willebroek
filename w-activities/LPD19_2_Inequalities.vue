...
// ——— PREDICTION ———
const showPrediction = computed(() => phase.value == 'predict')
...
function submitPrediction() {
  ...
  if (flipOk && opOk) {
    ...
    attemptCount.value++
    errorDetected.value = `Let op: het teken klapt ${lvl.flips ? 'WEL' : 'NIET'} om, en de eerste stap is ${lvl.multiStep ? 'de constante verplaatsen' : 'delen door de coëfficiënt van x'}.`
    feedback.value = { type: 'error', text: 'Pas je antwoorden aan.' }
  } else if (!flipOk) {
    attemptCount.value++
    errorDetected.value = `Let op het teken! Deel je door een ${lvl.flips ? 'negatief' : 'positief'} getal, dan klapt het teken ${lvl.flips ? 'WEL' : 'NIET'} om.`
    feedback.value = { type: 'error', text: 'Pas je voorspelling over het teken aan.' }
  } else {
    attemptCount.value++
    errorDetected.value = `De eerste stap is niet juist. ${lvl.multiStep ? 'Eerst de constante term verplaatsen.' : 'Je moet delen door de coëfficiënt van x.'}`
    feedback.value = { type: 'error', text: 'Kies een andere eerste stap.' }
  }
}
...
function checkSolution() {
  ...
  if ((dirFlip) && !valWrong) {
    errorDetected.value = `Juist getal, verkeerde RICHTING. ${lvl.flips ? 'Delen door een negatief getal klapt het teken OM!' : 'Het teken blijft hetzelfde.'}`
    feedback.value = { type: 'error', text: hints[idx] }
  } else if (inclWrong && !valWrong) {
    errorDetected.value = `Juiste richting, maar let op het verschil tussen < en ≤ (of > en ≥). Moet de grenswaarde erbij horen?`
    feedback.value = { type: 'error', text: hints[idx] }
  } else if (valWrong && !dirFlip && !inclWrong) {
    errorDetected.value = `De waarde klopt niet. Controleer je berekening.`
    feedback.value = { type: 'error', text: hints[idx] }
  } else {
    feedback.value = { type: 'error', text: hints[idx] }
  }
...
function checkReflection() {
  const a = reflectionAnswer.value ? reflectionAnswer.value.toLowerCase() : ''
  const conceptWords = ['ongelijkheid', 'teken', 'klapt', 'negatief', 'delen', 'vermenigvuldig', 'oplossen', 'getallenas', 'richting', 'grenswaarde', 'interval', 'controleren']
  const reasonWords = ['omdat', 'dus', 'daarom', 'want', 'als', 'dan', 'doordat', 'hierdoor', 'regel', 'waarom']
  ...
  attemptCount.value++
  if (!minLen) {
    errorDetected.value = 'Schrijf een volledige zin (minimaal 20 tekens).'
    feedback.value = { type: 'error', text: 'Probeer het opnieuw.' }
  } else if (!hasConcept) {
    errorDetected.value = 'Probeer vaktaal te gebruiken, zoals "ongelijkheid", "teken klapt om" of "negatief getal".'
    feedback.value = { type: 'error', text: 'Probeer het opnieuw.' }
  } else {
    errorDetected.value = 'Leg ook uit WAAROM het teken klapt, gebruik woorden zoals "omdat" of "daarom".'
    feedback.value = { type: 'error', text: 'Probeer het opnieuw.' }
  }
  return false
}
...
function checkNumberLine() {
  ...
  if (valOk && dirOk && incOk) {
    ...
    attemptCount.value++
    errorDetected.value = `De grenswaarde moet x = ${lvl.targetVal} zijn.`
    feedback.value = { type: 'error', text: 'Pas het aan op de as.' }
  } else if (!dirOk) {
    attemptCount.value++
    errorDetected.value = `Richting verkeerd. x ${lvl.targetDirection === 'right' ? '>' : '<'} ${lvl.targetVal}, dus pijl naar ${lvl.targetDirection === 'right' ? 'rechts' : 'links'}.`
    feedback.value = { type: 'error', text: 'Pas het aan op de as.' }
  } else {
    attemptCount.value++
    errorDetected.value = lvl.targetIncluded ? 'Gesloten bolletje (≤ of ≥) — grenswaarde hoort erbij.' : 'Open bolletje (< of >) — grenswaarde hoort er niet bij.'
    feedback.value = { type: 'error', text: 'Pas het aan op de as.' }
  }
}
      ...
      <div class="flex-col hidden w-full max-w-sm bg-white border-r-2 border-slate-200 shadow-inner-light md:flex z-10">
        <div ref="mainArea" tabindex="-1" class="flex-1 overflow-y-auto p-5 space-y-4 min-h-[300px]">
          ...
          <div class="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-xl">
            <button @click="showWorkedExample = !showWorkedExample" class="flex items-center gap-2 text-sm font-bold text-amber-700 w-full text-left">
              <PhLightbulb weight="fill" class="w-4 h-4" /> Voorbeelden: Ongelijkheden oplossen
            </button>
            <div v-if="showWorkedExample" class="mt-3 text-sm text-amber-800 space-y-2 animate-fadeIn">
              <div v-for="(ex, i) in exampleLevels" :key="i" class="bg-white p-3 rounded-lg border border-amber-200">
                <p class="font-mono font-bold text-sm">{{ ex.problem }} <span class="text-amber-700 font-normal">→ {{ ex.solution }}</span></p>
                <p class="text-xs text-slate-500 mt-1">{{ ex.note }}</p>
              </div>
              <p class="p-2 bg-amber-100 rounded text-xs"><strong>Let op:</strong> Deel/vermenigvuldig je met een NEGATIEF getal? Dan klapt het teken om!</p>
            </div>
          </div>
          ...
          <div v-if="phase === 'instructions'" class="bg-amber-50 rounded-xl border-2 border-amber-200 overflow-hidden">
            <div class="p-4">
              <p class="text-sm text-slate-600 mb-3">Je gaat ongelijkheden oplossen. Elk level heeft een andere uitdaging.</p>
            </div>
          ...
          <!-- BLOOM'S LABEL -->
          <div class="flex items-center gap-2">
            <span class="px-2 py-0.5 text-xs font-bold rounded-full bg-indigo-100 text-indigo-700">{{ levelLabels[currentInternalLevel] }}</span>
          ...
          <div v-if="phase === 'quickcheck'">
            <div class="bg-amber-50 rounded-xl border-2 border-amber-300 p-5">
              <div class="flex items-center gap-2 mb-4">
                <PhQuestion weight="fill" class="w-5 h-5 text-amber-600 shrink-0" />
                <span class="font-bold text-amber-800">Snap je het?</span>
              </div>
              <p class="text-sm text-amber-800 mb-4 p-3 bg-white rounded-xl border border-amber-200">
              ...
              <div class="flex flex-col gap-2 mb-4">
                <button v-for="opt in qcOptions" :key="opt.id" @click="qcAnswer = opt.id"
                  class="w-full text-left py-2.5 px-4 rounded-xl border-2 text-sm font-medium transition-all active:scale-[0.98]"
                  :class="qcAnswer === opt.id ? 'border-emerald-500 bg-amber-50 text-amber-800 shadow-sm' : 'border-slate-300 bg-white text-slate-600 hover:border-amber-300'">{{ opt.text }}</button>
              </div>
              ...
              <button @click="submitQuickCheck" :disabled="qcAnswer === null"
                class="w-full py-3 font-bold text-white rounded-xl bg-amber-600 hover:bg-amber-500 disabled:opacity-40 disabled:cursor-not-allowed transition-all active:scale-[0.98]">Bevestig</button>
            </div>
          </div>
          ...
          <div v-if="showWhy && !showReflection" class="mt-4 p-4 bg-indigo-50 border border-indigo-200 rounded-xl animate-fadeIn">
            <h4 class="text-sm font-bold text-indigo-700 mb-2 flex items-center gap-2">
              <PhLightbulb weight="fill" class="w-4 h-4" /> Waarom werkt dit?
            </h4>
            <p class="text-xs text-indigo-600">{{ whyText }}</p>
            <button @click="showReflection = true; showWhy = false" class="mt-3 text-xs font-bold text-indigo-700 underline">Naar reflectie</button>
          </div>

          <!-- === REFLECTION === -->
          <div v-if="phase === 'reflection' || showReflection">
            <div class="bg-indigo-50 rounded-xl border-2 border-indigo-200 p-5">
              <div class="flex items-center gap-2 mb-4">
                <PhLightbulb weight="fill" class="w-5 h-5 text-indigo-600 shrink-0" />
                <span class="font-bold text-indigo-800">Reflectie</span>
              </div>
              <p class="text-sm text-indigo-800 mb-4 p-3 bg-white rounded-xl border border-indigo-200">{{ currentLevelData.flips
                ? 'Waarom moet het teken omklappen bij delen/vermenigvuldigen met een negatief getal? Schrijf in je eigen woorden.'
                : 'Hoe controleer je of je een ongelijkheid juist hebt opgelost? Schrijf in je eigen woorden.' }}</p>
              ...
              <button @click="checkReflection()" :disabled="!reflectionAnswer || reflectionAnswer.length < 5"
                class="mt-3 w-full py-3 font-bold text-white rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed transition-all active:scale-[0.98]">Bevestig reflectie</button>
            </div>
          </div>

          <!-- === DONE === -->
          <!-- === LPD BEWIJS (DONE) === -->
          <div v-if="phase === 'done' || reflectDone">
            <div class="bg-emerald-50 rounded-xl border-2 border-emerald-200 p-5">
              <div class="flex items-center gap-2 mb-2">
                <PhCheckCircle weight="fill" class="w-5 h-5 text-emerald-600" />
                <span class="font-bold text-emerald-800 text-sm">LPD-bewijs</span>
              </div>
              <p class="text-sm text-emerald-700 leading-relaxed">{{ currentLevelData.flips
                ? 'Ik begrijp waarom het teken omklapt bij delen door een negatief getal: getallen wisselen van plaats rond 0 op de getallenas.'
                : 'Ik kan een ongelijkheid oplossen en controleren door een getal uit de oplossing in te vullen.' }}</p>
              <div class="flex items-center gap-2 mt-3">
                <input type="checkbox" id="lpd-ineq" v-model="reflectDone" class="w-4 h-4 accent-emerald-600" />
                <label for="lpd-ineq" class="text-xs text-emerald-700 font-medium">Ik bevestig dat ik dit beheers.</label>
              </div>
            </div>
          </div>
        ...
        <div class="p-4 bg-slate-50 border-t-2 border-slate-200 shrink-0">
          <div v-if="feedback.text"
               ...
               :class="{ 'bg-amber-50 text-amber-800 border-amber-300': feedback.type === 'success', 'bg-slate-50 text-slate-700 border-slate-300': feedback.type === 'error', 'bg-blue-50 text-blue-800 border-blue-300': feedback.type === 'info' }">
          ...
          <div class="flex items-center gap-3">
            ...
            <button v-if="!isCorrect && phase !== 'instructions' && phase !== 'predict'" @click="showHintAction()" class="flex-1 py-3 font-bold text-slate-600 rounded-xl border-2 border-slate-300 bg-white hover:bg-slate-50 transition-colors active:scale-[0.98] flex items-center justify-center gap-2">
              <PhLightbulb weight="fill" class="w-5 h-5 text-amber-500" />
              <span>{{ showHint ? 'Hint ' + hintCount : 'Hint' }}</span>
            </button>
            <button v-if="phase === 'done'" @click="handleNext" class="flex-1 py-3 font-bold text-white rounded-xl bg-amber-600 hover:bg-amber-500 flex items-center justify-center gap-2 transition-all active:scale-[0.98]">
        ...
        <div class="flex flex-col flex-1 p-6 overflow-y-auto items-center justify-center pattern-grid">
          <div class="w-full max-w-2xl bg-white p-8 rounded-xl shadow-md border border-slate-200">
            <div class="text-center mb-6">
              <div v-if="phase === 'done'" class="text-xl font-black text-amber-700 font-mono">
