...
function checkAnswer(ans) {
  ...
  if (ans === data.isTrue) {
    ...
    errorDetected.value = ""
    if (data.isTrue) {
      errorDetected.value = data.errorAnalysis.falseAnswer || `De ${data.inner} volgt wél alle regels van de ${data.outer}.`
    } else {
      errorDetected.value = data.errorAnalysis.trueAnswer || `De ${data.outer} is strenger dan de ${data.inner}.`
    }
...
function checkReflection() {
  ...
  const conceptWords = ['omdat', 'want', 'gelijk', 'hoeken', 'zijden', 'evenwijdig']
  if (ans.length > 10) {
