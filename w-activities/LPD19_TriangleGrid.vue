  ...
  if (isMatch) {
    ...
    if (attemptCount.value === 1) {
      ...
      if (val === data.triangle.base * data.triangle.height) {
        errorDetected.value = 'Je berekende basis × hoogte, maar vergeet niet te delen door 2. De driehoek is de helft van de rechthoek.'
      } else if (val === data.triangle.base + data.triangle.height) {
        errorDetected.value = 'Je moet vermenigvuldigen (basis × hoogte), niet optellen. Deel het resultaat door 2.'
      } else {
        errorDetected.value = 'Tel de hokjes in de driehoek of gebruik de formule: (basis × hoogte) / 2.'
      }
...
function checkReflection() {
  ...
  const conceptWords = ['omdat', 'want', 'door', 'helft', 'rechthoek', '2']
  if (ans.length > 10) {
    reflectionSubmitted.value = true
    feedback.value = { type: 'success', text: 'Mooie reflectie!' }
  } else {
    feedback.value = { type: 'info', text: 'Geef een wat uitgebreider antwoord en leg uit waarom.' }
  }
}
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <div v-if="showReflection" class="p-3 mb-3 bg-slate-100 border border-slate-200 rounded-lg animate-fadeIn">
              ...
              <button v-if="!reflectionSubmitted" @click="checkReflection"
