...
function checkWin() {
  ...
  if (isolatedLeft || isolatedRight) {
    ...
    attemptCount.value++
    errorDetected.value = "De variabele is nog niet geïsoleerd. Gebruik de tegengestelde bewerking aan BEIDE kanten om factoren weg te werken."
    feedback.value = { type: 'error', text: "Kies een andere bewerking. Druk op de Hint-knop als je vastzit." }
  }
}
...
function checkReflection() {
  ...
  if (keywords.some(k => ans.includes(k)) && ans.length > 8) {
    ...
    attemptCount.value++
    errorDetected.value = 'Je uitleg is niet volledig. Gebruik belangrijke termen zoals "beide kanten", "tegengestelde bewerking", of "isoleren".'
    feedback.value = { type: 'error', text: `Leg uit: welke bewerkingen heb je toegepast en waarom? Hint: ${currentLevelData.value.why}` }
  }
}
      ...
      <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <div v-if="showWorkedExample" class="bg-amber-50 p-4 rounded-lg mb-4 border-l-4 border-amber-400 animate-fadeIn">
            ...
            <p class="text-sm font-bold text-amber-700 mb-1">b = A/h</p>
          ...
          <div v-if="showReflection" class="mt-4 p-4 bg-amber-50 rounded-lg border-l-4 border-amber-400 animate-fadeIn">
            ...
            <p class="text-xs text-amber-700 mb-2">Leg uit welke bewerkingen je hebt toegepast en waarom die de variabele isoleren.</p>
      ...
      <div class="flex flex-col flex-1 overflow-hidden bg-slate-50">
        <div class="flex flex-col flex-1 p-6 overflow-y-auto items-center justify-center relative pattern-grid">
          <div class="bg-white p-12 rounded-xl shadow-md border-4 flex items-center gap-6 min-w-[300px] justify-center transition-all"
            :class="isCorrect ? 'border-amber-400' : 'border-amber-200'">
            <div class="text-3xl font-mono font-black text-slate-700 min-w-[60px] text-right" :class="{ 'text-amber-600': isCorrect && isIsolated(stateL, currentLevelData.targetVar) }">
            ...
            <div class="text-3xl font-mono font-black text-slate-700 min-w-[60px] text-left" :class="{ 'text-amber-600': isCorrect && isIsolated(stateR, currentLevelData.targetVar) }">
