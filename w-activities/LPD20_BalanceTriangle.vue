...
const checkReflection = () => { showReflection.value = false; }
      ...
      <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <div class="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-xl">
            ...
            <div v-if="showWorkedExample" class="mt-3 text-sm text-amber-800 space-y-2 animate-fadeIn">
              ...
              <p class="text-amber-600 font-bold">Jouw beurt: lees de missie en kies het juiste type lijn.</p>
            </div>
          </div>
          ...
          <div class="p-4 border border-slate-200 bg-slate-50 rounded-xl shadow-inner">
            ...
            <div class="flex flex-col gap-2">
              <button @click="setLines('hoogte')" :disabled="isCorrect" class="py-4 px-4 rounded-lg border-2 font-bold text-left transition-all active:scale-[0.98] shadow-sm" :class="activeLines === 'hoogte' ? (isCorrect ? 'bg-amber-500 border-emerald-600 text-white' : 'bg-red-500 border-red-600 text-white') : 'bg-white border-slate-300 text-slate-600 hover:bg-slate-50'">Hoogtelijnen</button>
              <button @click="setLines('zwaarte')" :disabled="isCorrect" class="py-4 px-4 rounded-lg border-2 font-bold text-left transition-all active:scale-[0.98] shadow-sm" :class="activeLines === 'zwaarte' ? (isCorrect ? 'bg-amber-500 border-emerald-600 text-white' : 'bg-red-500 border-red-600 text-white') : 'bg-white border-slate-300 text-slate-600 hover:bg-slate-50'">Zwaartelijnen</button>
              <button @click="setLines('middel')" :disabled="isCorrect" class="py-4 px-4 rounded-lg border-2 font-bold text-left transition-all active:scale-[0.98] shadow-sm" :class="activeLines === 'middel' ? (isCorrect ? 'bg-amber-500 border-emerald-600 text-white' : 'bg-red-500 border-red-600 text-white') : 'bg-white border-slate-300 text-slate-600 hover:bg-slate-50'">Middelloodlijnen</button>
              <button @click="setLines('bissec')" :disabled="isCorrect" class="py-4 px-4 rounded-lg border-2 font-bold text-left transition-all active:scale-[0.98] shadow-sm" :class="activeLines === 'bissec' ? (isCorrect ? 'bg-amber-500 border-emerald-600 text-white' : 'bg-red-500 border-red-600 text-white') : 'bg-white border-slate-300 text-slate-600 hover:bg-slate-50'">Bissectrices (Deellijnen)</button>
            </div>
          </div>
          ...
          <div v-if="isCorrect && !showReflection" class="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
            <h4 class="text-sm font-bold text-amber-700 mb-2">Waarom werkt dit?</h4>
            <p class="text-xs text-amber-600">{{ currentLevelData.why }}</p>
            <button @click="showReflection = true" class="mt-3 text-xs font-bold text-amber-700 underline">Bevestig dat je dit begrijpt</button>
          </div>

          <!-- Reflection Question -->
          <div v-if="showReflection && !lpdConfirmed" class="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
            <h4 class="text-sm font-bold text-amber-700 mb-2">Reflectie</h4>
            <p class="text-xs text-amber-600 mb-2">Waarom bepaalt het snijpunt van bepaalde lijnen een bijzonder punt van de driehoek? Welke eigenschap van die lijnen zorgt daarvoor?</p>
            <textarea v-model="reflectionAnswer" class="w-full p-3 text-sm border border-amber-300 rounded-lg bg-white" rows="2" placeholder="Typ je antwoord..."></textarea>
            <button @click="lpdConfirmed = true" :disabled="reflectionAnswer.length < 5" class="mt-2 px-4 py-2 bg-indigo-600 text-white text-sm font-bold rounded-lg disabled:opacity-50 hover:bg-amber-500">Bevestig</button>
          </div>

          <!-- LPD Evidence -->
          <div v-if="lpdConfirmed && !isCorrect" class="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
            <h4 class="text-sm font-bold text-amber-700 mb-2 flex items-center gap-2"><PhTarget weight="fill" class="w-4 h-4" /> LPD-bewijs</h4>
            <p class="text-xs text-amber-600 mb-2">Ik kan de vier merkwaardige lijnen (hoogtelijn, zwaartelijn, middelloodlijn, bissectrice) herkennen en toepassen in contextproblemen.</p>
            <div class="flex items-center gap-2">
              <input type="checkbox" id="lpd-balance" v-model="lpdConfirmed" class="w-4 h-4 accent-emerald-600" />
              <label for="lpd-balance" class="text-xs text-amber-700 font-medium">Ik bevestig dat ik dit begrijp en kan toepassen.</label>
            </div>
          </div>
        </div>
        ...
        <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
          ...
          <div class="flex items-center gap-4">
            ...
            <button v-if="isCorrect" @click="handleNext" class="flex items-center justify-center flex-1 gap-2 py-4 font-bold text-white transition-all rounded-lg shadow-md bg-amber-600 hover:bg-amber-500 active:scale-[0.98]">
