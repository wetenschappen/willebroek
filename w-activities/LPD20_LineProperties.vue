...
import { PhX, PhCheckCircle, PhWarningCircle, PhArrowRight, PhScan, PhArrowClockwise, PhLightbulb, PhTarget, PhBrain, PhPencilSimpleLine } from '@phosphor-icons/vue'
...
function showHint() {
  ...
  feedback.value = { type: 'info', text: getHint(currentLevelData.value) }
}
...
function checkPrediction() {
  ...
  predictionCorrect.value = parseInt(predictionAnswer.value) === currentInternalLevel.value + 1
  predictionSubmitted.value = true
  if (predictionCorrect.value) {
    showPrediction.value = false
    feedback.value = { type: 'success', text: 'Juist! Je herkent de lijn al. Laten we controleren.' }
  } else {
    attemptCount.value++
    feedback.value = { type: 'error', text: 'Niet juist. Bekijk de eigenschappen van de rode lijn goed.' }
  }
}

function checkReflection() {
  const text = reflectionAnswer.value.toLowerCase().trim()
  const hasConcept = text.includes('lijn') || text.includes('eigenschap') || text.includes('hoekpunt') || text.includes('loodrecht')
  ...
  if (hasConcept && hasReason && text.length > 15) {
    ...
    feedback.value = { type: 'success', text: 'Uitstekende reflectie! Je begrijpt de determinatietabel.<br/><br/><strong>LPD 20 bewezen:</strong> Je herkent lijnen aan hun eigenschappen en kan verklaren waarom de combinatie uniek is.' }
    isCorrect.value = true
  } else {
    attemptCount.value++
    feedback.value = { type: 'info', text: 'Leg uit waarom elke lijn een unieke combinatie heeft. Gebruik woorden zoals "eigenschap" en "lijn" en leg het verband uit.' }
  }
}

function resetActivityState() {
  isCorrect.value = false; celebrationDone.value = false; isChecked.value = false; attemptCount.value = 0; hintCount.value = 0
  ...
  showPrediction.value = true; predictionAnswer.value = null; predictionSubmitted.value = false; predictionCorrect.value = false
  showWhy.value = false; whyText.value = ''
  showReflection.value = false; reflectionAnswer.value = ''; reflectionDone.value = false
}
...
function handleNext() {
  if (currentInternalLevel.value === totalInternalLevels.value - 1 && !showReflection.value && !reflectionDone.value) {
    ...
    showWhy.value = false
    isCorrect.value = false
    feedback.value = { type: 'info', text: 'Reflecteer op wat je leerde over de merkwaardige lijnen.' }
    return
  }
      ...
      <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <div v-if="showPrediction" class="p-5 border-2 border-amber-300 bg-amber-50 rounded-xl mb-4 animate-fadeIn">
            ...
            <p class="text-sm text-amber-800 mb-3">
              Wat voor type lijn is dit volgens jou?<br />
              <span class="font-bold">1</span> = Hoogtelijn, <span class="font-bold">2</span> = Zwaartelijn, <span class="font-bold">3</span> = Middelloodlijn, <span class="font-bold">4</span> = Bissectrice
            </p>
            <div class="flex items-center gap-2">
              <input type="number" v-model.number="predictionAnswer" placeholder="?" min="1" max="4"
              ...
              <button @click="checkPrediction" :disabled="predictionAnswer === null || predictionAnswer < 1 || predictionAnswer > 4"
          ...
          <div v-if="!showPrediction" class="text-center bg-amber-50 p-4 border border-amber-200 rounded-xl shadow-sm mb-6 animate-fadeIn">
            <span class="text-xs font-bold text-amber-700 uppercase tracking-widest block mb-2">{{ levelLabels ? levelLabels[currentInternalLevel] + ' — ' : '' }}{{ currentLevelData ? currentLevelData.goalText || 'Opdracht' : 'Opdracht' }}</span>
          </div>
          ...
          <div v-if="showReflection && !reflectionDone" class="mt-4 p-4 bg-indigo-50 border border-indigo-200 rounded-xl animate-fadeIn">
            ...
            <p class="text-sm text-slate-700 mb-3">Waarom heeft elk type lijn een unieke combinatie van eigenschappen (hoekpunt, loodrecht, midden)? Hoe helpt de determinatietabel je om de juiste lijn te kiezen?</p>
            <textarea v-model="reflectionAnswer" :disabled="reflectionDone"
              ...
              rows="3" placeholder="Elke lijn heeft een unieke combinatie omdat..."></textarea>
        ...
        <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
          ...
          <div class="flex items-center gap-4">
            ...
            <button v-if="!isCorrect && !showPrediction" @click="checkAnswer" class="flex-1 py-4 font-bold text-white transition-all rounded-lg shadow-md bg-slate-800 hover:bg-slate-900">Controleer Kaart</button>
