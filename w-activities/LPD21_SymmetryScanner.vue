...
function checkAnswer() {
  ...
  if (testState.value === 'none') {
    isCorrect.value = false
    attemptCount.value++
    errorDetected.value = 'Gebruik de scanner-knoppen om de figuur te testen voor je antwoord kiest!'
    feedback.value = { type: 'error', text: 'Voer eerst een test uit.' }
    return
  }
  ...
  if (userVal === target) {
    ...
    if (userVal === data.commonMistake) {
      errorDetected.value = data.mistakeMsg
      feedback.value = { type: 'error', text: 'Bekijk de testresultaten zorgvuldig.' }
    } else if (userVal === 'none' && target !== 'none') {
      errorDetected.value = 'Deze figuur heeft wel symmetrie! Test beide soorten opnieuw. ' + data.hintLevels[0]
      feedback.value = { type: 'error', text: 'Bekijk de testresultaten zorgvuldig.' }
    } else if (userVal === 'both' && target === 'none') {
      errorDetected.value = 'Deze figuur heeft geen symmetrie. Geen enkele test zou moeten passen. ' + data.hintLevels[0]
      feedback.value = { type: 'error', text: 'Bekijk de testresultaten zorgvuldig.' }
    } else if (userVal === 'line') {
      errorDetected.value = (data.correctAns === 'point' ? 'Lijnsymmetrie werkt niet — de gespiegelde versie past niet. Probeer puntsymmetrie.' : (data.errLine || 'Kijk goed. Past hij na het spiegelen echt perfect?'))
      feedback.value = { type: 'error', text: 'Bekijk de testresultaten zorgvuldig.' }
    } else if (userVal === 'point') {
      errorDetected.value = (data.correctAns === 'line' ? 'Puntsymmetrie werkt niet — na 180° draaien past hij niet. Probeer lijnsymmetrie.' : (data.errPoint || 'Kijk goed. Past hij na 180° draaien echt perfect?'))
      feedback.value = { type: 'error', text: 'Bekijk de testresultaten zorgvuldig.' }
    } else {
      errorDetected.value = 'Niet helemaal... ' + data.hintLevels[0]
      feedback.value = { type: 'error', text: 'Bekijk de testresultaten zorgvuldig.' }
    }
  }
}
...
function checkReflection() {
  ...
  if (!reflectionAnswer.value || reflectionAnswer.value.length < 5) {
    attemptCount.value++
    errorDetected.value = 'Schrijf een volledige zin om verder te gaan.'
    feedback.value = { type: 'error', text: 'Je reflectie is te kort.' }
    return
  }
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            <div v-if="feedback.text" class="flex items-start gap-4 p-4 mb-4 text-sm font-medium rounded-lg animate-fadeIn" role="status" aria-live="polite" aria-atomic="true"
                 :class="{'bg-amber-100 text-amber-800': feedback.type === 'success', 'bg-slate-100 text-slate-700 border border-slate-200': feedback.type === 'error', 'bg-slate-100 text-slate-700': feedback.type === 'info'}">
