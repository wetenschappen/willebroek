...
function drawLine() { if (isCorrect.value) return; if (!circlesDrawn.value) { attemptCount.value++; feedback.value = { type: 'error', text: 'Eerst de snijpunten zoeken! ' + getHint() }; return }; lineDrawn.value = true; isCorrect.value = true; hintCount.value = 0; feedback.value = { type: 'success', text: 'Prima! De lijn door de twee snijpunten is altijd exact de middelloodlijn.' }
  const whyExplanation = 'De snijpunten van twee even grote cirkels liggen altijd op de middelloodlijn. Omdat beide cirkels dezelfde straal hebben, is elk snijpunt even ver van A als van B. Een punt dat even ver van A als van B ligt, ligt op de middelloodlijn. Twee punten bepalen precies een rechte.'
...
function checkPrediction() {
  ...
  predictionCorrect.value = parseInt(predictionAnswer.value) === 3  // 3 steps: radius, circles, line
  predictionSubmitted.value = true
  if (predictionCorrect.value) {
    showPrediction.value = false
    feedback.value = { type: 'success', text: 'Juist! Precies 3 stappen. Laten we het doen.' }
  } else {
    attemptCount.value++
    feedback.value = { type: 'error', text: `Denk na: je moet de passer instellen, cirkels tekenen en dan de snijpunten verbinden. Hoeveel stappen zijn dat?` }
  }
}

function checkReflection() {
  const text = reflectionAnswer.value.toLowerCase().trim()
  const hasConcept = text.includes('cirkel') || text.includes('straal') || text.includes('afstand') || text.includes('middelloodlijn')
  ...
  if (hasConcept && hasReason && text.length > 15) {
    ...
    feedback.value = { type: 'success', text: 'Uitstekende reflectie! Je begrijpt de constructie.<br/><br/><strong>LPD 22 bewezen:</strong> Je kan een middelloodlijn construeren en verklaren waarom de constructie werkt.' }
    isCorrect.value = true
  } else {
    attemptCount.value++
    feedback.value = { type: 'info', text: 'Leg uit waarom de twee cirkels exact dezelfde straal moeten hebben. Gebruik woorden als "cirkel", "straal" en "afstand".' }
  }
}

function resetActivityState() { isCorrect.value = false; celebrationDone.value = false; isChecked.value = false; attemptCount.value = 0; hintCount.value = 0; feedback.value = { type: 'info', text: 'Klik op de eerste knop om te beginnen.' }; compassRadiusSet.value = false; circlesDrawn.value = false; lineDrawn.value = false; showPrediction.value = true; predictionAnswer.value = null; predictionSubmitted.value = false; predictionCorrect.value = false; showWhy.value = false; whyText.value = ''; showReflection.value = false; reflectionAnswer.value = ''; reflectionDone.value = false }
...
function handleNext() {
  if (currentInternalLevel.value === totalInternalLevels.value - 1 && !showReflection.value && !reflectionDone.value) {
    ...
    feedback.value = { type: 'info', text: 'Reflecteer op wat je leerde over de middelloodlijn-constructie.' }
    return
  }
        ...
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <div v-if="showPrediction" class="p-5 border-2 border-amber-300 bg-amber-50 rounded-xl mb-4 animate-fadeIn">
            ...
            <p class="text-sm text-amber-800 mb-3">
              Hoeveel stappen heeft de constructie van een middelloodlijn?<br />
              <span class="font-bold">Stap 1</span>: Stel passer in, <span class="font-bold">Stap 2</span>: Teken cirkels, <span class="font-bold">Stap 3</span>: Verbind snijpunten
            </p>
            <div class="flex items-center gap-2">
              <input type="number" v-model.number="predictionAnswer" placeholder="?" min="1" max="5"
              ...
              <button @click="checkPrediction" :disabled="predictionAnswer === null || predictionAnswer < 1 || predictionAnswer > 5"
          ...
          <div v-if="!showPrediction" class="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-xl">
            ...
            <div v-if="showWorkedExample" class="mt-3 text-sm text-amber-800 space-y-2 animate-fadeIn">
              ...
              <p class="font-bold text-amber-600">Resultaat: de lijn staat loodrecht op AB en gaat door het midden.</p>
            </div>
          </div>
          ...
          <div class="p-4 border border-slate-200 bg-slate-50 rounded-xl shadow-inner">
            ...
            <div class="flex flex-col gap-2">
              ...
              <button @click="drawLine" :disabled="isCorrect || lineDrawn" class="py-2 px-4 rounded-lg border-2 font-bold text-left flex items-center justify-between" :class="lineDrawn ? 'bg-amber-50 border-amber-400 text-amber-700' : 'bg-slate-800 border-slate-900 text-white hover:bg-slate-700'">
                <span>3. Verbind de snijpunten</span><PhCheckCircle v-if="lineDrawn" weight="fill" class="text-amber-500" />
              </button>
            </div>
          </div>
          ...
          <div v-if="showReflection && !reflectionDone" class="mt-4 p-4 bg-indigo-50 border border-indigo-200 rounded-xl animate-fadeIn">
            ...
            <p class="text-sm text-slate-700 mb-3">Waarom moeten de twee cirkels exact dezelfde straal hebben? Wat gebeurt er als de stralen verschillen?</p>
            <textarea v-model="reflectionAnswer" :disabled="reflectionDone"
              ...
              rows="3" placeholder="De cirkels moeten dezelfde straal hebben omdat..."></textarea>
