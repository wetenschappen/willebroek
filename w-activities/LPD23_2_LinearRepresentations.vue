...
function checkPrediction() {
  ...
  if (Math.abs(Number(predictedSlope.value) - correctA) < 0.01) {
    ...
    errorDetected.value = errType
    feedback.value = { type: 'error', text: 'Pas je voorspelling aan.' }
  }
}
...
function checkAnswer() {
  ...
  if (aOk && bOk) {
    ...
    if (attemptCount.value >= 3) {
      errorDetected.value = `Zowel a als b kloppen niet.`
      feedback.value = { type: 'info', text: `Let op: a = ${lvl.targetA} en b = ${lvl.targetB}. Gebruik de hint-knop voor de methode.` }
    } else {
      errorDetected.value = `Zowel a als b kloppen niet. Begin met a = Δy/Δx te berekenen.`
      feedback.value = { type: 'error', text: 'Pas de schuifregelaars aan.' }
    }
  } else if (!aOk) {
    ...
    if (Math.abs(userA.value - (lvl.targetB !== 0 ? lvl.points[1].y / lvl.points[1].x : 0)) < 0.01) {
      errorDetected.value = 'Je hebt a berekend als y/x in plaats van Δy/Δx. Gebruik TWEE punten: a = (y₂−y₁)/(x₂−x₁).'
      feedback.value = { type: 'error', text: 'Pas de schuifregelaars aan.' }
    } else if (Math.abs(userA.value + lvl.targetA) < 0.01) {
      errorDetected.value = 'Het teken van a is omgekeerd. Controleer of de lijn stijgt (a>0) of daalt (a<0).'
      feedback.value = { type: 'error', text: 'Pas de schuifregelaars aan.' }
    } else {
      errorDetected.value = `a = ${userA.value} is niet correct. De juiste waarde heeft te maken met Δy/Δx = ${lvl.targetA}.`
      feedback.value = { type: 'error', text: 'Pas de schuifregelaars aan.' }
    }
  } else {
    // b wrong
    if (userB.value === 0 && lvl.targetB !== 0) {
      errorDetected.value = `b is niet 0. De lijn gaat niet door de oorsprong. b = y − ax. Vul een punt in: b = ${lvl.points[0].y} − ${userA.value}·${lvl.points[0].x}.`
      feedback.value = { type: 'error', text: 'Pas de schuifregelaars aan.' }
    } else {
      errorDetected.value = `b = ${userB.value} is niet correct. Tip: b = y − ax. Neem een punt uit de tabel en reken na.`
      feedback.value = { type: 'error', text: 'Pas de schuifregelaars aan.' }
    }
  }
}
...
function checkReflection() {
  ...
  if (keywords.some(k => ans.includes(k)) && ans.length > 10) {
    ...
    attemptCount.value++
    errorDetected.value = 'Denk na: wat maakt dat een rechte lijn overal dezelfde helling heeft? Gebruik termen zoals "constant", "Δy/Δx" of "verhouding".'
