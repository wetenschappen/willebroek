...
import { PhX, PhCheckCircle, PhWarningCircle, PhArrowRight, PhTriangle, PhArrowClockwise, PhArrowsLeftRight, PhLightbulb, PhTarget, PhBrain, PhPencilSimpleLine } from '@phosphor-icons/vue'
...
function resetActivityState() { isCorrect.value = false; celebrationDone.value = false; isChecked.value = false; feedback.value = { type: 'info', text: 'Beweeg de slider.' }; userAns.value = ''; swingAngle.value = 90; attemptCount.value = 0; hintCount.value = 0; showPrediction.value = true; predictionAnswer.value = null; predictionSubmitted.value = false; predictionCorrect.value = false; showWhy.value = false; whyText.value = ''; showReflection.value = false; reflectionAnswer.value = ''; reflectionDone.value = false }

function showHint() {
  hintCount.value++
  const idx = Math.min(hintCount.value - 1, 2)
  const hints = [
    'Beweeg de slider en kijk hoeveel keer de blauwe lijn de basis raakt.',
    'Bij 30° en 45°: hoeveel verschillende posities kan de zijde aannemen?',
    'Bij 90°: waarom is er maar één mogelijkheid?'
  ]
...
function checkPrediction() {
  ...
  predictionCorrect.value = predictionAnswer.value === currentOptions.value.correctAns
  predictionSubmitted.value = true
  if (predictionCorrect.value) {
    showPrediction.value = false
    feedback.value = { type: 'success', text: 'Juist ingeschat! Beweeg de slider om het te bewijzen.' }
  } else {
    attemptCount.value++
    feedback.value = { type: 'error', text: 'Niet juist. Beweeg de slider en kijk goed naar de blauwe lijn.' }
  }
}

function checkReflection() {
  const text = reflectionAnswer.value.toLowerCase().trim()
  const hasConcept = text.includes('zzh') || text.includes('congruentie') || text.includes('driehoek') || text.includes('zijde')
  ...
  if (hasConcept && hasReason && text.length > 15) {
    ...
    feedback.value = { type: 'success', text: 'Uitstekende reflectie! Je begrijpt de ZZH-valstrik.<br/><br/><strong>LPD 24 bewezen:</strong> Je kan uitleggen waarom ZZH geen congruentiekenmerk is.' }
    isCorrect.value = true
  } else {
    attemptCount.value++
    feedback.value = { type: 'info', text: 'Leg uit waarom ZZH geen garantie is voor congruentie. Gebruik "ZZH" en "congruentie".' }
  }
}
...
function checkAnswer() {
  ...
  if (userAns.value === currentOptions.value.correctAns) {
    isCorrect.value = true
    hintCount.value = 0
    const whyExplanation = 'Bij ZZH (Zijde-Zijde-Hoek) kan de derde zijde op twee manieren passen (behalve bij 90°). Dit komt doordat de onbekende zijde de cirkel op twee punten kan snijden. Daarom is ZZH geen congruentiekenmerk — in tegenstelling tot ZHZ, ZZZ en ZHH.'
...
function handleNext() {
  if (currentInternalLevel.value === totalInternalLevels - 1 && !showReflection.value && !reflectionDone.value) {
    ...
    feedback.value = { type: 'info', text: 'Reflecteer op wat je leerde over de ZZH-valstrik.' }
    return
  }
        ...
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <div v-if="showPrediction" class="p-5 border-2 border-amber-300 bg-amber-50 rounded-xl mb-4 animate-fadeIn">
            ...
            <p class="text-sm text-amber-800 mb-3">
              {{ currentOptions.userOpt1 }}<br />
              of<br />
              {{ currentOptions.userOpt2 }}<br />
              Wat denk jij?
            </p>
            <div class="flex gap-2">
              <button @click="predictionAnswer = 'uniek'; checkPrediction()"
              ...
              <button @click="predictionAnswer = 'twee'; checkPrediction()"
                      class="flex-1 px-3 py-2 text-sm font-bold text-amber-800 bg-white border-2 border-amber-300 rounded-lg hover:bg-amber-100 transition-colors active:scale-[0.98]">
                Twee
              </button>
            </div>
          ...
          <div v-if="!showPrediction" class="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-xl">
            ...
            <div v-if="showWorkedExample" class="mt-3 text-sm text-amber-800 space-y-2 animate-fadeIn">
              ...
              <p>Bij ZZH kan de derde zijde op twee manieren passen (behalve bij 90°).</p>
              <p class="font-bold text-amber-600">Test het zelf met de slider!</p>
            </div>
          </div>

          <div v-if="!showPrediction" class="p-4 border border-red-200 bg-red-50 rounded-xl shadow-inner mb-4">
            <label class="block text-sm font-bold text-red-900 mb-2">Conclusie:</label>
            <div class="text-xs text-amber-800 mb-2">{{ levelLabels ? levelLabels[currentInternalLevel] + ' — ' : '' }}{{ currentLevelData.angleLabel }}° hoek</div>
          ...
          <div v-if="showReflection && !reflectionDone" class="mt-4 p-4 bg-indigo-50 border border-indigo-200 rounded-xl animate-fadeIn">
            ...
            <p class="text-sm text-slate-700 mb-3">Waarom maakt de positie van de hoek ten opzichte van de zijden het verschil tussen ZHZ en ZZH?</p>
            <textarea v-model="reflectionAnswer" :disabled="reflectionDone"
              ...
              rows="3" placeholder="De positie van de hoek maakt verschil omdat..."></textarea>
