...
function resetActivityState() {
  ...
  isCorrect.value = false; celebrationDone.value = false; showReflection.value = false; showWhy.value = false; whyText.value = ''; reflectionAnswer.value = ''
      ...
      <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <div v-if="showWhy && !showReflection && !reflectionDone" class="mt-4 p-4 bg-indigo-50 border border-indigo-200 rounded-xl animate-fadeIn">
            <h4 class="text-sm font-bold text-indigo-700 mb-2 flex items-center gap-2"><PhLightbulb weight="fill" class="w-4 h-4" /> Waarom werkt dit?</h4>
            <p class="text-xs text-indigo-600 leading-relaxed">{{ whyText }}</p>
          ...
          <!-- Reflection -->
          <div v-if="showReflection" class="mt-4 p-4 bg-indigo-50 border border-indigo-200 rounded-xl animate-fadeIn">
            <h4 class="text-sm font-bold text-indigo-700 mb-2 flex items-center gap-2"><PhLightbulb weight="fill" class="w-5 h-5" /> Reflectie</h4>
            <p v-if="currentLevelData.isInequality" class="text-xs text-indigo-600 mb-2">
        ...
        <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
          <div v-if="feedback.text" class="flex items-start gap-3 p-3 mb-4 text-sm font-medium rounded-lg animate-fadeIn border-l-4"
            ...
            :class="{ 'bg-amber-50 text-amber-800 border-amber-400': feedback.type === 'success', 'bg-red-100 text-red-800 border-red-400': feedback.type === 'error', 'bg-amber-100 text-amber-800 border-amber-400': feedback.type === 'info' }">
