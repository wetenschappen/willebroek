...
function resetActivityState() { isCorrect.value = false; celebrationDone.value = false; isChecked.value = false; feedback.value = { type: 'info', text: 'Kies de juiste vormen.' }; topViewAns.value = ''; frontViewAns.value = ''; attemptCount.value = 0; hintCount.value = 0; showPrediction.value = true; predictionAnswer.value = null; predictionSubmitted.value = false; predictionCorrect.value = false; showWhy.value = false; whyText.value = ''; showReflection.value = false; reflectionAnswer.value = ''; reflectionDone.value = false; if (levels.value.length === 0) levels.value = shuffleArray(shapeTemplates) }

function showHint() {
  hintCount.value++
  const idx = Math.min(hintCount.value - 1, 2)
  const hints = ['Bekijk het object van boven: welke vorm zie je?', 'Bekijk het object van voor: welke hoogte zie je?', 'Combineer beide aanzichten voor het complete beeld.' ]
...
function checkPrediction() {
  ...
  if (predictionCorrect.value) {
    ...
    feedback.value = { type: 'success', text: 'Goed! Laten we de aanzichten controleren.' }
  } else {
    attemptCount.value++
    feedback.value = { type: 'error', text: 'Denk na over de vorm van boven en van voren.' }
  }
}

function checkReflection() {
  const text = reflectionAnswer.value.toLowerCase().trim()
  const hasConcept = text.includes('aanzicht') || text.includes('boven') || text.includes('voor') || text.includes('3d')
  ...
  if (hasConcept && hasReason && text.length > 15) {
    ...
    feedback.value = { type: 'success', text: 'Uitstekende reflectie!<br/><br/><strong>LPD 26 bewezen:</strong> Je kan 3D-objecten herkennen aan hun aanzichten.' }
    isCorrect.value = true
  } else {
    attemptCount.value++
    feedback.value = { type: 'info', text: 'Leg uit hoe aanzichten helpen om een 3D-vorm te herkennen.' }
  }
}
...
function handleNext() {
  if (currentInternalLevel.value === totalInternalLevels - 1 && !showReflection.value && !reflectionDone.value) {
    ...
    feedback.value = { type: 'info', text: 'Reflecteer op wat je leerde over 3D-aanzichten.' }
    return
  }
      ...
      <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <div v-if="showPrediction" class="p-5 border-2 border-amber-300 bg-amber-50 rounded-xl mb-4 animate-fadeIn">
            ...
            <p class="text-sm text-amber-800 mb-3">Wat is het bovenaanzicht van dit object?</p>
            <div class="flex gap-2 flex-wrap">
              <button @click="predictionAnswer = true; checkPrediction()" class="px-3 py-2 text-sm font-bold text-amber-800 bg-white border-2 border-amber-300 rounded-lg hover:bg-amber-100">Ik herken de vorm!</button>
              <button @click="predictionAnswer = false; checkPrediction()" class="px-3 py-2 text-sm font-bold text-amber-800 bg-white border-2 border-amber-300 rounded-lg hover:bg-amber-100">Ik twijfel</button>
            </div>
          </div>
          ...
          <div v-if="!showPrediction" class="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-xl">
            ...
            <div v-if="showWorkedExample" class="mt-3 text-sm text-amber-800 space-y-2 animate-fadeIn">
              ...
              <p class="font-bold text-amber-600">Een aanzicht is altijd een platte 2D-vorm!</p>
            </div>
          </div>
          ...
          <div v-if="showReflection && !reflectionDone" class="mt-4 p-4 bg-indigo-50 border border-indigo-200 rounded-xl animate-fadeIn">
            ...
            <p class="text-sm text-slate-700 mb-3">Waarom verandert het aanzicht afhankelijk van de richting waaruit je kijkt?</p>
            <textarea v-model="reflectionAnswer" :disabled="reflectionDone"
              ...
              rows="3" placeholder="Het aanzicht verandert omdat..."></textarea>
          ...
          <div class="w-full max-w-4xl flex justify-center gap-12 items-center">
            ...
            <div class="flex flex-col gap-6">
              <div class="flex items-center gap-4">
                ...
                <div class="w-24 h-24 bg-white rounded-xl shadow-sm border-2 border-slate-300 flex items-center justify-center" :class="topViewAns === currentLevelData.topAns && isCorrect ? 'border-emerald-500 bg-amber-50' : ''">
              ...
              <div class="flex items-center gap-4">
                ...
                <div class="w-24 h-24 bg-white rounded-xl shadow-sm border-2 border-slate-300 flex items-center justify-center" :class="frontViewAns === currentLevelData.frontAns && isCorrect ? 'border-emerald-500 bg-amber-50' : ''">
