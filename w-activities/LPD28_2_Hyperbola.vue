...
import { PhX, PhCheckCircle, PhWarningCircle, PhArrowRight, PhTrendDown, PhArrowClockwise, PhLightbulb, PhTarget, PhBrain, PhPencilSimpleLine } from '@phosphor-icons/vue'
import MathText from './MathText.vue'
import SuccessCelebration from './SuccessCelebration.vue'

const props = defineProps({
  isOpen: Boolean,
  title: { type: String, default: 'Omgekeerd Evenredig (f(x) = c/x)' },
...
function generateLevel(idx) {
  if (idx === 0) {
    return {
      goalText: 'Positieve hyperbool',
      q: 'Maak c positief zodat de takken in kwadrant I en III liggen.',
      checkFn: (c) => c > 0,
      successMsg: 'Juist! Positieve c = takken in I en III.',
      startC: -3,
      targetPoint: null,
      why: 'Bij een positieve c (c > 0) liggen de takken van de hyperbool in kwadrant I (rechtsboven) en III (linksonder). Dit komt doordat x en y hetzelfde teken hebben: als x positief is, is y = c/x ook positief; als x negatief is, is y ook negatief.',
      hintTexts: [
        'In welke kwadranten moeten de takken liggen?',
        'Kijk naar het teken van c. Wat gebeurt er met y = c/x als c positief is?',
        'Probeer c naar een positieve waarde te schuiven, zoals c = 2.'
      ],
      errorHint: 'De takken zitten nu in II en IV. Dat betekent dat c negatief is. Probeer c > 0 te maken.'
    }
  } else if (idx === 1) {
    return {
      goalText: 'Negatieve hyperbool',
      q: 'Maak c negatief zodat de takken in kwadrant II en IV liggen.',
      checkFn: (c) => c < 0,
      successMsg: 'Perfect! Negatieve c = takken in II en IV.',
      startC: 3,
      targetPoint: null,
      why: 'Bij een negatieve c (c < 0) liggen de takken in kwadrant II (linksboven) en IV (rechtsonder). x en y hebben nu een verschillend teken: als x positief is, is y = c/x negatief; als x negatief is, is y positief.',
      hintTexts: [
        'De takken moeten in kwadrant II en IV liggen. Wat betekent dat voor c?',
        'Als c negatief is, hebben x en y tegengestelde tekens. Probeer c < 0.',
        'Schuif c naar een negatieve waarde, zoals c = -2.'
      ],
      errorHint: 'c is nu positief (takken in I en III). Probeer c < 0 te maken.'
    }
  }
  ...
  const targetC = tx * ty
  return {
    goalText: 'Het Doelpunt',
    q: `Zorg dat de hyperbool door (${tx}, ${ty}) gaat.`,
    checkFn: (c) => c === targetC,
    successMsg: `Geweldig! c = ${targetC}. y = c/x => ${ty} = ${targetC}/${tx}. Klopt!`,
    startC: 1,
    targetPoint: { x: tx, y: ty },
    why: `Het punt (${tx}, ${ty}) ligt op de hyperbool als voldaan is aan y = c/x. Dus c = x · y = ${tx} · ${ty} = ${targetC}. Het product x · y is constant voor elk punt op de hyperbool.`,
    hintTexts: [
      'Het punt (tx, ty) moet op de grafiek liggen. Wat is het verband tussen x, y en c?',
      'Voor een punt op de hyperbool geldt: c = x · y. Bereken het product.',
      `c = ${tx} × ${ty} = ${targetC}. Zoek deze waarde met de slider.`
    ],
    errorHint: `Het punt moet op de hyperbool liggen. Gebruik c = x · y = ${tx} · ${ty}.`
  }
}

const currentLevelData = computed(() => levels.value[currentInternalLevel.value] || levels.value[0])
const cValue = ref(1)
...
const graphPath = computed(() => {
  ...
  for (let x = -10; x <= 10; x += 0.2) {
    ...
    const sx = x * 10
    const sy = -y * 10
    if (x < 0) pts1.push(`${sx},${sy}`)
    else pts2.push(`${sx},${sy}`)
  }
...
watch(cValue, (newVal) => {
  if (!isCorrect.value && levels.value.length > 0) {
    ...
    if (t && t.checkFn(newVal)) {
      isCorrect.value = true
      feedback.value = {
        type: 'success',
        text: t.successMsg + ' De hyperbool f(x) = c/x heeft twee takken die nooit de assen raken (asymptoten). Het teken van c bepaalt in welke kwadranten de takken liggen: c>0 = I en III, c<0 = II en IV. De grootte van c bepaalt hoe ver de takken van de oorsprong liggen.'
      }
      showWhy.value = true
      whyText.value = t.why
    }
...
function resetActivityState() {
  ...
  feedback.value = { type: 'info', text: 'Beweeg de slider.' }
  ...
  errorDetected.value = ''
  if (levels.value.length === 0) levels.value = [0, 1, 2].map(i => generateLevel(i))
...
function checkReflection() {
  ...
  const conceptWords = ['tak', 'kwadrant', 'asymptoot', 'hyperbool', 'c/x', 'omgekeerd', 'evenredig', 'product', 'constant', 'teken', 'positief', 'negatief']
  const reasonWords = ['omdat', 'dus', 'daarom', 'want', 'als', 'dan', 'doordat', 'hierdoor']
  ...
  else if (!hasConcept) feedback.value = { type: 'error', text: 'Probeer vaktaal te gebruiken, zoals "hyperbool", "asymptoot" of "kwadrant".' }
  else feedback.value = { type: 'error', text: 'Leg ook uit WAAROM het werkt, gebruik woorden zoals "omdat" of "dus".' }
  return false
}
      ...
      <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <div v-if="showPrediction" class="p-5 border-2 border-amber-300 bg-amber-50 rounded-xl mb-6 animate-fadeIn">
            ...
            <p class="text-sm text-amber-700 mb-3">{{ currentInternalLevel === 0 ? 'Wordt c positief of negatief om takken in I en III te krijgen?' : currentInternalLevel === 1 ? 'Wordt c positief of negatief om takken in II en IV te krijgen?' : 'Hoe kun je c berekenen als de hyperbool door een punt moet gaan?' }}</p>
            <div class="flex gap-2">
              <button @click="predictionAnswer = 'positief'; submitPrediction()" class="flex-1 py-2 px-4 text-sm font-bold rounded-lg border-2 transition-colors" :class="predictionAnswer === 'positief' ? 'bg-amber-500 text-white border-amber-500' : 'bg-white text-amber-700 border-amber-300 hover:bg-amber-100'">Positief (c > 0)</button>
              <button @click="predictionAnswer = 'negatief'; submitPrediction()" class="flex-1 py-2 px-4 text-sm font-bold rounded-lg border-2 transition-colors" :class="predictionAnswer === 'negatief' ? 'bg-amber-500 text-white border-amber-500' : 'bg-white text-amber-700 border-amber-300 hover:bg-amber-100'">Negatief (c < 0)</button>
              <button @click="predictionAnswer = 'product'; submitPrediction()" class="flex-1 py-2 px-4 text-sm font-bold rounded-lg border-2 transition-colors" :class="predictionAnswer === 'product' ? 'bg-amber-500 text-white border-amber-500' : 'bg-white text-amber-700 border-amber-300 hover:bg-amber-100'">c = x · y</button>
            </div>
          </div>
          ...
          <div class="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-xl">
            ...
            <div v-if="showWorkedExample" class="mt-3 text-sm text-amber-800 space-y-2 animate-fadeIn">
              ...
              <p class="font-bold text-amber-600">De x-as en y-as zijn asymptoten: de takken naderen maar raken ze nooit.</p>
            </div>
          </div>
          ...
          <div v-if="showHint && !isCorrect" class="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
            ...
            <p class="text-sm text-amber-800">{{ currentLevelData.hintTexts[hintLevel] }}</p>
          ...
          <div v-if="reflectionDone" class="mt-4 p-4 bg-emerald-50 border border-emerald-200 rounded-xl animate-fadeIn">
            ...
            <div class="flex items-center gap-2">
              <input type="checkbox" id="lpd-hyper" v-model="reflectionDone" class="w-4 h-4 accent-emerald-600" />
              <label for="lpd-hyper" class="text-xs text-emerald-700 font-medium">Ik bevestig dat ik dit beheers.</label>
            </div>
          </div>
        </div>
        ...
        <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
          ...
          <div class="flex items-center gap-4">
            ...
            <button v-if="!isCorrect" class="flex-1 py-4 font-bold text-slate-400 text-center rounded-lg shadow-md bg-slate-100 cursor-default">Schuif naar antwoord</button>
