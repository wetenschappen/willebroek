...
watch([valA, valP, valQ], () => {
  ...
  sliderTimer = setTimeout(() => {
    if (!isCorrect.value) {
      if (signErrorP) {
        errorDetected.value = `Als de top op x = ${t.p} ligt, dan is p = ${t.p}, niet -${t.p}.`
        feedback.value = {
          type: 'error',
          text: `<strong>Let op het teken!</strong> In f(x) = a(x - p)² + q is de top (p, q). Jij hebt p = ${valP.value} maar de top van de doelparabool is (${t.p}, ${t.q}).<br>Kijk: f(x) = a(x - ${t.p})² + q.`
        }
      } else if (signErrorQ) {
        errorDetected.value = `Let op het teken van q!`
        feedback.value = {
          type: 'error',
          text: `De top is (${t.p}, ${t.q}) — de y-coordinaat q = ${t.q}. Jij hebt q = ${valQ.value}.`
        }
      } else if (swappedPQ) {
        errorDetected.value = `Niet verwarren!`
        feedback.value = {
          type: 'error',
          text: `p is de x-coordinaat en q is de y-coordinaat van de top. Jij hebt p = ${valP.value} en q = ${valQ.value}, maar de top is (${t.p}, ${t.q}).`
        }
      } else {
        errorDetected.value = "De parabool ligt nog niet goed."
        feedback.value = {
          type: 'error',
