...
function onValueChange() {
  ...
  if (tv === 0) {
    ...
    if (attemptCount.value % 3 === 0) {
      ...
      errorDetected.value = "Nog niet in evenwicht. " + hint
      feedback.value = { type: 'error', text: 'Pas x verder aan.' }
    }
  }
}
...
function submitLPD() {
  ...
  if (lpdAnswer.value.length >= 20) {
    lpdDone.value = true; emit('complete')
  } else {
    attemptCount.value++
    errorDetected.value = "Je bewijs moet minimaal 20 tekens bevatten."
    feedback.value = { type: 'error', text: 'Vul je antwoord aan.' }
  }
}
    ...
    <main class="flex flex-1 overflow-hidden">
      <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 md:flex z-10">
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <details class="bg-amber-50 rounded-xl p-4 border border-amber-200 mb-4">
            ...
            <div class="mt-3 text-sm space-y-2">
              <p class="flex items-center gap-2"><PhCheckCircle weight="bold" class="w-4 h-4 text-amber-500 shrink-0" /> Vraag: x + 3 = 8. Wat is x?</p>
              <p class="flex items-center gap-2"><PhCheckCircle weight="bold" class="w-4 h-4 text-amber-500 shrink-0" /> Stel x = 3: 3 + 3 = 6, niet 8. Te weinig.</p>
              <p class="flex items-center gap-2"><PhCheckCircle weight="bold" class="w-4 h-4 text-amber-500 shrink-0" /> Stel x = 5: 5 + 3 = 8. Evenwicht! x = 5.</p>
            </div>
          </details>
          ...
          <div v-if="!predictionMade" class="bg-amber-50 rounded-xl p-4 border border-amber-200 mb-4">
            ...
            <p class="text-xs text-amber-600 mb-3">Hoe groot denk je dat x is?</p>
            <div class="space-y-2">
        ...
        <div class="p-4 bg-slate-50 border-t border-slate-200 shrink-0">
          <div v-if="feedback.text" class="flex items-start gap-3 p-3 mb-3 text-sm rounded-lg animate-fadeIn border-l-4"
