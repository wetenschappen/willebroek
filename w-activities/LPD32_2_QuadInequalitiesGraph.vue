...
function checkReflection() {
  ...
  if (reflectAns.value.trim().length > 0) {
    ...
    attemptCount.value++
    errorDetected.value = "Je reflectie mag niet leeg zijn."
    feedback.value = { type: 'error', text: 'Schrijf een antwoord.' }
  }
}
...
function checkAnswer() {
  ...
  if (rootsCorrect && aboveCorrect && includedCorrect) {
    ...
    if (!rootsCorrect) {
      errorDetected.value = `De snijpunten van de parabool met de x-as zijn x = ${t.targetRoots[0]} en x = ${t.targetRoots[1]}.`
      feedback.value = {
        type: 'error',
        text: `<strong>De nulwaarden kloppen niet.</strong> <br><span class="text-xs text-slate-500">Zoek waar de grafiek de x-as snijdt.</span><br>${getHintText(attemptCount.value)}`
      }
    } else if (aboveBelowConfusion) {
      errorDetected.value = `In de ongelijkheid staat f(x) ${t.targetAbove ? '>' : '<'} 0. Dat betekent: ${t.targetAbove ? 'boven de x-as (positief)' : 'onder de x-as (negatief)'}.`
      feedback.value = {
        type: 'error',
        text: `<strong>Let op of de grafiek boven of onder de x-as ligt!</strong><br>Jij hebt het ${userAbove.value ? 'bovenste' : 'onderste'} gedeelte geselecteerd, maar het moet het ${t.targetAbove ? 'bovenste' : 'onderste'} zijn.<br>${getHintText(0)}<br><span class="text-xs text-slate-500">Tip: een dalparabool (a>0) is positief buiten de nulwaarden. Een bergparabool (a<0) is positief tussen de nulwaarden.</span>`
      }
    } else if (signMixup) {
      errorDetected.value = `De nulwaarden moet${t.targetIncluded ? '' : ' niet'} meegerekend worden. ${t.targetIncluded ? 'Gebruik gesloten bollen (≤, ≥).' : 'Gebruik open bollen (<, >).'}`
      feedback.value = {
        type: 'error',
        text: `<strong>Let op ≤ vs < (en ≥ vs >)!</strong><br>Ongelijkheid: ${t.inequality}.`
      }
    } else {
      errorDetected.value = "Nog niet correct."
      feedback.value = {
        type: 'error',
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            <div v-if="feedback.text"
                 ...
                 :class="{
                   ...
                   'bg-slate-100 text-slate-700 border border-slate-200': feedback.type === 'error',
