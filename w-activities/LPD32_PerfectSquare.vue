...
function checkAnswer() {
  ...
  if (userAns.value === 'correct') {
    if (isSplit.value) {
      ...
      attemptCount.value++
      const errMsg = 'Splits eerst het vierkant om de delen te zien!'
    ...
    const errMsg = 'Dat is NET de fout die we willen aantonen! (a+b)² is NIET a²+b². De twee oranje rechthoeken (ab+ab=2ab) ontbreken.'
    errorDetected.value = errMsg
    feedback.value = {
      type: 'error',
      text: errMsg
    ...
    attemptCount.value++
    const errMsg = 'Bijna! Er zijn TWEE oranje rechthoeken, niet een. Tel opnieuw: ab + ab = 2ab.'
    errorDetected.value = errMsg
    feedback.value = {
      type: 'error',
      text: errMsg
...
<div v-if="isOpen" class="fixed inset-0 z-50 flex flex-col items-center justify-center bg-slate-50 text-slate-800">
  ...
  <div class="relative flex flex-col w-screen h-screen overflow-hidden shadow-xl bg-white">
    ...
    <!-- PREDICTION GATE -->
    <div v-if="showPrediction" class="flex-1 flex items-center justify-center p-8 bg-slate-50">
