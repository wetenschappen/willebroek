...
function checkPrediction() {
  ...
  if (predictedFirstStep.value === 'subConst') {
    ...
    attemptCount.value++
    errorDetected.value = 'Let op: er staat nog een los getal. Eerst dat wegwerken, dan pas delen.'
    feedback.value = { type: 'error', text: 'Kies een andere stap.' }
  } else if (predictedFirstStep.value === 'add') {
    attemptCount.value++
    errorDetected.value = 'We willen x isoleren, niet groter maken. Wat moet je wegdoen?'
    feedback.value = { type: 'error', text: 'Kies een andere stap.' }
  } else {
    attemptCount.value++
    errorDetected.value = 'Vermenigvuldigen maakt het complexer. Welke GETALLEN staan in de weg?'
    feedback.value = { type: 'error', text: 'Kies een andere stap.' }
  }
}
  ...
  if (op === 'sub') {
    if (constLeft.value >= 1) {
      ...
      if (stepAfterConst.value) {
        attemptCount.value++
        errorDetected.value = 'De constante is al weg. Deel nu door het aantal x-en.'
        feedback.value = { type: 'error', text: 'Kies een andere bewerking.' }
      } else {
        attemptCount.value++
        errorDetected.value = 'Er zijn geen losse blokjes meer om weg te halen bij x.'
        feedback.value = { type: 'error', text: 'Kies een andere bewerking.' }
        if (attemptCount.value >= 3) {
           const hint = getHintText(attemptCount.value)
           errorDetected.value += " " + hint.text
        }
      }
    }
  } else if (op === 'subAll') {
    if (constLeft.value > 0) {
      ...
      attemptCount.value++
      errorDetected.value = 'Er zijn geen losse blokjes meer.'
      feedback.value = { type: 'error', text: 'Kies een andere bewerking.' }
    }
  } else if (op === 'add') {
    attemptCount.value++
    constLeft.value += 1; constRight.value += 1
    errorDetected.value = 'Toevoegen maakt het groter — we willen x isoleren!'
    feedback.value = { type: 'error', text: 'Kies een andere bewerking.' }
    if (attemptCount.value >= 3) {
       const hint = getHintText(attemptCount.value)
       errorDetected.value += " " + hint.text
    ...
    if (constLeft.value === 0 && !stepAfterConst.value) {
      attemptCount.value++
      errorDetected.value = 'Haal eerst de losse blokjes weg.'
      feedback.value = { type: 'error', text: 'Kies een andere bewerking.' }
      return
    }
    if (xCount.value === lvl.divisor) {
      ...
      attemptCount.value++
      errorDetected.value = `Deel door het juiste aantal. Je hebt ${xCount.value} x-en, deel door ${xCount.value}.`
      feedback.value = { type: 'error', text: 'Kies de juiste deling.' }
      if (attemptCount.value >= 3) {
         const hint = getHintText(attemptCount.value)
         errorDetected.value += " " + hint.text
      }
    ...
    errorDetected.value = 'Vermenigvuldigen van beide kanten is toegestaan, maar hier maakt het x groter, niet kleiner.'
    feedback.value = { type: 'error', text: 'Kies een andere bewerking.' }
    if (attemptCount.value >= 3) {
       const hint = getHintText(attemptCount.value)
       errorDetected.value += " " + hint.text
...
function triggerHint() {
  ...
  hintCount.value++
  feedback.value = getHintText(hintLevel.value * 2 + 3)
}
...
function checkReflection() {
  ...
  if (hasKeyword && ans.length > 5) {
    ...
    attemptCount.value++
    errorDetected.value = "Je uitleg is niet volledig. Denk aan constante/losse blokjes en het isoleren van x."
    feedback.value = { type: 'error', text: 'Probeer uit te leggen met de woorden: constanten, wegwerken, delen.' }
  }
}
      ...
      <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
        <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
          ...
          <div v-if="showWhy && !showReflection" class="mt-4 p-4 bg-amber-50 rounded-lg border-l-4 border-amber-400 animate-fadeIn">
            <p class="font-bold text-amber-800 text-sm mb-1">Waarom werkt de balansmethode?</p>
            <p class="text-xs text-amber-700 leading-relaxed">Het =-teken is als een balans in evenwicht. <strong>Alles wat je aan de ene kant doet, moet je aan de andere kant ook doen.</strong> Eerst haal je de constante weg (aftrekken), daarna deel je door de coëfficiënt van x. Zo blijft de vergelijking waar terwijl je x isoleert.</p>
          ...
          <div v-if="showReflection" class="mt-4 p-4 bg-amber-50 rounded-lg border-l-4 border-amber-400 animate-fadeIn">
            ...
            <p class="text-xs text-amber-700 mb-2">Waarom mag je de constante wegwerken VOOR je deelt? Leg uit in je eigen woorden.</p>
            <input v-model="reflectionAnswer" @keyup.enter="checkReflection" class="w-full p-2 text-sm border border-amber-300 rounded-lg bg-white outline-none focus:border-indigo-500" placeholder="Typ je uitleg..." />
        ...
        <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
          <div v-if="feedback.text" class="flex items-start gap-3 p-3 mb-4 text-sm font-medium rounded-lg animate-fadeIn border-l-4" role="status" aria-live="polite" aria-atomic="true"
            :class="{
              ...
              'bg-slate-50 text-slate-700 border-slate-400': feedback.type === 'error',
              'bg-blue-50 text-blue-800 border-blue-400': feedback.type === 'info',
            }">
        ...
        <div class="flex flex-col flex-1 p-6 overflow-y-auto items-center justify-center relative pattern-grid">
          <div class="w-full max-w-2xl flex flex-col items-center gap-6">
            ...
            <div class="font-mono font-black text-3xl text-slate-700 bg-white px-8 py-4 rounded-full shadow-sm border-2 border-slate-200 flex gap-3 items-center transition-all">
              ...
              <span v-if="isCorrect" class="text-amber-600 ml-4">→ x = {{ constRight }}</span>
            </div>
