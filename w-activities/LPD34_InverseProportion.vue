          ...
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div v-if="showWorkedExample" class="bg-amber-50 p-4 rounded-lg mb-4 border-l-4 border-amber-400 animate-fadeIn">
              ...
              <div class="overflow-x-auto mb-2">
                <table class="w-full text-sm border-collapse bg-white rounded"><thead><tr class="bg-amber-100"><th class="p-1 border">Breedte (x)</th><th class="p-1 border">Lengte (y)</th><th class="p-1 border">x × y</th></tr></thead>
                  <tbody>
                    <tr v-for="row in workedExamples[0].rows" :key="row.x"><td class="p-1 border text-center font-bold">{{ row.x }}</td><td class="p-1 border text-center font-bold">{{ row.y }}</td><td class="p-1 border text-center font-bold text-amber-600">{{ row.x * row.y }}</td></tr>
                  </tbody>
                </table>
              </div>
