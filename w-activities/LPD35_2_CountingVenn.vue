...
function checkReflection() { reflectionDone.value=true }
  ...
  <div class="relative flex w-full max-w-6xl max-h-[90vh] bg-white rounded-2xl shadow-2xl overflow-hidden">
      ...
      <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-amber-50 border border-amber-200 rounded-xl p-6 mb-6">
          ...
          <div class="space-y-4">
            ...
            <div class="p-3 bg-white rounded-lg border border-amber-100">
              <p class="font-bold text-amber-700">Boomdiagram (twee opeenvolgende keuzes):</p>
    ...
    <div v-if="showPredictionGate&&!predictionMade" class="absolute inset-0 z-20 bg-white overflow-y-auto p-8">
      <div class="max-w-xl mx-auto text-center">
        <div class="p-3 rounded-full bg-amber-100 w-16 h-16 flex items-center justify-center mx-auto mb-4"><PhTarget class="w-8 h-8 text-amber-600" weight="fill" /></div>
        ...
        <div class="flex flex-col gap-3">
          <button @click="handlePrediction('venn')" class="p-4 border-2 border-amber-200 rounded-xl hover:bg-amber-50 hover:border-amber-400 font-bold text-slate-700 transition-all">Venn-diagram (overlappende cirkels)</button>
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div class="p-3 mb-4 bg-amber-50 border border-amber-200 rounded-lg">
              <p class="text-xs font-bold text-amber-700">{{ currentInternalLevel < 2 ? 'Venn-diagram: sleep de getallen' : 'Boomdiagram: vul de aantallen in' }}</p>
            </div>

            <div v-if="showWhySection" class="p-4 mb-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
              <h4 class="font-bold text-amber-800 mb-2 text-sm">Waarom werkt dit?</h4>
              <p class="text-xs text-amber-700">Venn-diagrammen helpen om overlappingen tussen verzamelingen te visualiseren. Boomdiagrammen tonen opeenvolgende keuzes en hun kansen. Beide maken telproblemen overzichtelijk en helpen om dubbeltellingen te vermijden.</p>
            </div>
            ...
            <div v-if="showReflection&&!reflectionDone" class="p-4 mb-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
              <h4 class="font-bold text-amber-800 mb-3 text-sm">Reflectie</h4>
              <p class="text-xs text-amber-700 mb-3"><strong>Waarom</strong> kies je een Venn-diagram boven een boomdiagram (of omgekeerd)? Welk type probleem past bij welk diagram?</p>
              <button @click="checkReflection" class="px-4 py-2 bg-indigo-600 text-white text-sm font-bold rounded-lg hover:bg-amber-500 transition-colors">Ik heb nagedacht</button>
            </div>

            <div v-if="showLPDEvidence&&!lpdDone" class="p-4 mb-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
              <h4 class="font-bold text-amber-800 mb-3 text-sm">LPD 35: Bewijs van beheersing</h4>
              <p class="text-xs text-amber-700 mb-3">Je hebt bewezen dat je telproblemen kan oplossen met diagrammen. Vul aan: "Bij twee overlappende groepen gebruik ik een _____ diagram. Bij opeenvolgende keuzes gebruik ik een _____ diagram. In beide gevallen moet de som van alle delen gelijk zijn aan ____."</p>
              <button @click="handleLPDDone" class="px-4 py-2 bg-amber-600 text-white text-sm font-bold rounded-lg hover:bg-amber-500 transition-colors">Ik kan dit uitleggen</button>
            </div>
          </div>
          ...
          <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
            ...
            <div class="flex items-center gap-4">
              ...
              <button v-if="isCorrect" @click="handleNext" class="flex items-center justify-center flex-1 gap-2 py-4 font-bold text-white transition-all rounded-lg shadow-md bg-amber-600 hover:bg-amber-500 active:scale-[0.98]"><span>{{ currentInternalLevel<totalInternalLevels-1?'Volgend Level':'Afronden' }}</span><PhArrowRight weight="bold" /></button>
            ...
            <div v-else class="relative w-full max-w-lg min-h-[350px] bg-white rounded-xl shadow-md border border-slate-200 p-6">
              ...
              <div class="flex justify-center gap-6">
                ...
                <div class="flex flex-col items-center gap-2">
                  ...
                  <div class="flex gap-3 mt-2">
                    <div class="flex flex-col items-center gap-1">
                      <input type="number" :value="treeInputs.aa" @input="updateTreeField('aa', $event.target.value)" class="w-16 h-10 text-center font-bold border-2 border-amber-300 rounded-lg focus:border-emerald-500 outline-none" placeholder="?" />
                      <span class="text-xs text-amber-600">{{ currentLevel.pref1 }}</span>
                    </div>
                    <div class="flex flex-col items-center gap-1">
                      <input type="number" :value="treeInputs.ab" @input="updateTreeField('ab', $event.target.value)" class="w-16 h-10 text-center font-bold border-2 border-amber-300 rounded-lg focus:border-emerald-500 outline-none" placeholder="?" />
                      <span class="text-xs text-amber-600">{{ currentLevel.pref2 }}</span>
                    </div>
                  </div>
                </div>
                <!-- Right branch -->
                <div class="flex flex-col items-center gap-2">
                  ...
                  <div class="flex gap-3 mt-2">
                    <div class="flex flex-col items-center gap-1">
                      <input type="number" :value="treeInputs.ba" @input="updateTreeField('ba', $event.target.value)" class="w-16 h-10 text-center font-bold border-2 border-amber-300 rounded-lg focus:border-emerald-500 outline-none" placeholder="?" />
                      <span class="text-xs text-amber-600">{{ currentLevel.pref1 }}</span>
                    </div>
                    <div class="flex flex-col items-center gap-1">
                      <input type="number" :value="treeInputs.bb" @input="updateTreeField('bb', $event.target.value)" class="w-16 h-10 text-center font-bold border-2 border-amber-300 rounded-lg focus:border-emerald-500 outline-none" placeholder="?" />
                      <span class="text-xs text-amber-600">{{ currentLevel.pref2 }}</span>
                    </div>
                  </div>
                </div>
