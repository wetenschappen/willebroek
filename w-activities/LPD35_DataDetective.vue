...
function checkAnswer(ans) {
  ...
  if (ans === currentLevel.value.correctAnswer) {
    ...
    if (attemptCount.value === 1) {
      ...
      errorDetected.value = ans === true ? 'Let op de dagen die afwijken van de trend.' : 'Let op of er wel een afwijking is.'
      feedback.value = { type: 'error', text: misdirection }
    } else if (attemptCount.value === 2) {
      errorDetected.value = `Let op het verschil tussen "algemene trend" en "elke dag".`
      feedback.value = {
        type: 'error',
      ...
      errorDetected.value = `Het juiste antwoord is: de bewering ${correctLabel}.`
      feedback.value = {
        type: 'error',
...
function checkReflection() {
  ...
  if (reflectionAnswer.value.trim().length < 5) {
    attemptCount.value++
    errorDetected.value = "Geef een iets uitgebreider antwoord over hoe je zeker bent."
