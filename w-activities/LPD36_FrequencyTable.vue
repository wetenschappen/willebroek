...
function checkAbsoluteFreqs() {
  ...
  if (!allFilled) {
    errorDetected.value = "Vul alle velden in de tabel in."
    feedback.value = { type: 'error', text: hintLevels1[Math.min(attemptCount.value - 1, 2)] }
    return
  }

  // Error analysis: check if total is wrong (double counting)
  if (totalEntered !== lvl.total) {
    errorDetected.value = `De som van je frequenties is ${totalEntered}, maar de totale data heeft ${lvl.total} items. Je hebt waarschijnlijk dubbel geteld of iets overgeslagen.`
    feedback.value = {
      type: 'error',
      text: hintLevels1[Math.min(attemptCount.value - 1, 2)]
    }
    hintCount.value++
    return
  }
  ...
  if (allCorrect) {
    ...
    const errorText = attemptCount.value < 3
      ? hintLevels1[Math.min(attemptCount.value - 1, 2)]
      : `De juiste frequenties: ${lvl.freq.map(f => `${f.category}: ${f.count}`).join(', ')}.`
    errorDetected.value = "Niet alle frequenties kloppen."
...
function checkRelativeFreqs() {
  ...
  if (!allFilled) {
    errorDetected.value = "Vul alle relatieve frequenties in."
    feedback.value = { type: 'error', text: hintLevels2[Math.min(attemptCount.value - 1, 2)] }
    return
  }

  // Error analysis: check if they add to ~1
  if (Math.abs(totalRel - 1) > 0.05) {
    errorDetected.value = `De som van relatieve frequenties moet 1 (of 100%) zijn, maar jouw som is ${totalRel.toFixed(2)}.`
    feedback.value = {
      type: 'error',
      text: `Relatieve frequenties geven het deel van het geheel aan, dus samen moeten ze het geheel vormen. ${hintLevels2[Math.min(attemptCount.value - 1, 2)]}`
    }
    hintCount.value++
    return
  }
  ...
  if (allCorrect) {
    ...
    const errorText = attemptCount.value < 3
      ? hintLevels2[Math.min(attemptCount.value - 1, 2)]
      : `Relatieve frequenties: ${lvl.relFreq.map(f => `${f.category}: ${(f.rel * 100).toFixed(0)}%`).join(', ')}.`
    errorDetected.value = "Niet alle relatieve frequenties kloppen."
    feedback.value = { type: 'error', text: errorText }
    hintCount.value++
  }
}
...
function checkCumulativeFreqs() {
  ...
  if (!allFilled) {
    errorDetected.value = "Vul alle cumulatieve frequenties in."
    feedback.value = { type: 'error', text: hintLevels3[Math.min(attemptCount.value - 1, 2)] }
    return
  }
  ...
  if (notIncreasing) {
    errorDetected.value = `Cumulatieve frequenties moeten steeds groter worden (of gelijk blijven).`
    feedback.value = {
      type: 'error',
      text: `Ze zijn de optelsom van alle voorgaande frequenties. ${hintLevels3[0]}`
    }
    hintCount.value++
    return
  }
  ...
  if (allCorrect && medianCorrect) {
    ...
    if (attemptCount.value < 3) {
      errorDetected.value = `De cumulatieve frequenties kloppen, maar de mediaan niet.`
      feedback.value = {
        type: 'error',
        text: `De mediaan is de waarde op de ${Math.ceil(half)}e positie van ${lvl.total}. In welke categorie valt die positie?`
      }
    } else {
      errorDetected.value = `Mediaan = ${lvl.medianCat}.`
      feedback.value = {
        type: 'error',
        text: `Totaal = ${lvl.total}, helft = ${half}. De categorie waar de ${Math.ceil(half)}e waarde valt is de mediaan.`
      }
    }
    hintCount.value++
  } else {
    const errorText = attemptCount.value < 3
      ? hintLevels3[Math.min(attemptCount.value - 1, 2)]
      : `Cumulatieve frequenties: ${lvl.cumFreq.map(f => `${f.category}: ${f.cum}`).join(', ')}.`
    errorDetected.value = "Niet alle cumulatieve frequenties kloppen."
    feedback.value = { type: 'error', text: errorText }
    hintCount.value++
  }
}
...
function checkReflection() {
  ...
  if (reflectionAnswer.value.trim().length < 5) {
    ...
    attemptCount.value++
    errorDetected.value = "Je reflectie is te kort."
      ...
      <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 shadow-inner-light md:flex z-10">
        ...
        <div class="p-6 bg-slate-50 border-t border-slate-200 shrink-0">
          <div v-if="feedback.text"
               class="flex items-start gap-4 p-4 mb-4 text-sm font-medium rounded-lg animate-fadeIn border-l-4"
