  ...
  <div class="relative flex w-full max-w-6xl max-h-[90vh] bg-white rounded-2xl shadow-2xl overflow-hidden">
    ...
    <div v-if="showPredictionGate&&!predictionMade" class="absolute inset-0 z-20 bg-white overflow-y-auto p-8">
      <div class="max-w-xl mx-auto text-center">
        <div class="p-3 rounded-full bg-amber-100 w-16 h-16 items-center justify-center mx-auto mb-4 flex"><PhTarget class="w-8 h-8 text-amber-600" weight="fill" /></div>
        ...
        <div class="flex flex-col gap-3">
          <button @click="handlePrediction('veelvouden')" class="p-4 border-2 border-amber-200 rounded-xl hover:bg-amber-50 hover:border-amber-400 font-bold text-slate-700 transition-all">Start met priemgetallen en streep hun veelvouden weg</button>
          ...
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div class="p-4 border border-slate-200 bg-slate-50 rounded-xl">
              ...
              <div class="flex flex-col gap-3 font-mono text-sm">
                ...
                <div class="flex items-center gap-4"><div class="w-8 h-8 rounded-full border-4 border-emerald-500 bg-amber-50 flex items-center justify-center text-amber-600 font-bold">1</div><span class="text-amber-600">Priemgetal! (2 kliks)</span></div>
              </div>
            </div>
            ...
            <div v-if="showWhySection" class="p-4 mt-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
              ...
              <p class="text-xs text-amber-700">De Zeef van Eratosthenes is efficiënt omdat elk samengesteld getal exact een keer wordt doorgestreept: bij de kleinste priemdeler. Hierdoor hoef je niet elk getal apart te testen. Dit is een elegant algoritme dat al meer dan 2200 jaar oud is!</p>
            </div>
            ...
            <div v-if="showReflection&&!reflectionDone" class="p-4 mt-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
              ...
              <p class="text-xs text-amber-700 mb-3"><strong>Waarom</strong> is de Zeef van Eratosthenes sneller dan elk getal apart testen op priem zijn? Wat is het verschil in aanpak?</p>
            ...
            <div v-if="showLPDEvidence&&!lpdDone" class="p-4 mt-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
              <h4 class="font-bold text-amber-800 mb-3 text-sm">LPD 37: Bewijs van beheersing</h4>
              <p class="text-xs text-amber-700 mb-3">Je hebt de Zeef van Eratosthenes uitgevoerd. Leg uit: "Ik start bij het eerste priemgetal ____, omcirkel het, en streep alle ____ door. Dan ga ik naar het volgende niet-doorgestreepte getal en herhaal. Op het einde zijn alle overblijvende ____ getallen de ____."</p>
            ...
            <div class="grid grid-cols-5 gap-4 bg-white p-8 rounded-xl shadow-md border border-slate-200">
              <button v-for="(state,idx) in numStates" :key="idx" @click="cycleState(idx)" :disabled="isCorrect"
                ...
                :class="{
                  ...
                  'bg-amber-50 border-4 border-emerald-500 text-amber-600 rounded-full scale-110 shadow-[0_0_15px_rgba(16,185,129,0.4)]': state==='circled'
                }">
