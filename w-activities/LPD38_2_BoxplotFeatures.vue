...
watch([valMin, valQ1, valMed, valQ3, valMax], () => {
  ...
  if (!(valMin.value <= valQ1.value && valQ1.value <= valMed.value && valMed.value <= valQ3.value && valQ3.value <= valMax.value)) {
    if (!feedback.value.text.includes('volgorde')) {
      ...
      errorDetected.value = 'Minimum ≤ Q1 ≤ Mediaan ≤ Q3 ≤ Maximum.'
      feedback.value = { type: 'error', text: 'Let op de volgorde van de kwartielen.' }
    }
    return
  } else if (feedback.value.type === 'error' && feedback.value.text.includes('volgorde')) {
    // Clear error if ordering is fixed
    feedback.value = { type: 'info', text: '' }
    errorDetected.value = ""
  }
...
function checkReflection() {
  ...
  if (reflectionAnswer.value.trim().length < 5) {
    attemptCount.value++
    errorDetected.value = "Je reflectie is onvolledig."
