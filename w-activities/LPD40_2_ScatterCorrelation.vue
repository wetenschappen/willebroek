...
const levelLabels = ['Herinneren', 'Begrijpen', 'Toepassen', 'Analyseren', 'Evalueren']

const Bloom = computed(() => {
  const labels = ['Herinneren', 'Begrijpen', 'Toepassen', 'Analyseren', 'Evalueren']
...
function selectAnswer(answer) {
  ...
  if (answer === currentLevel.value.targetCategory) {
    ...
    if (attemptCount.value < 3) {
      ...
      errorDetected.value = "Dit is niet het juiste verband."
      ...
      if (currentLevel.value.targetCategory.includes('Geen') && !answer.includes('Geen')) {
        ...
        errorDetected.value = "Er is een duidelijke uitschieter die het patroon verstoort."
      } else {
        errorMsg += 'Kijk naar de richting (stijgend/dalend) en hoe dicht de punten bij een lijn liggen.'
        errorDetected.value = "Beoordeel de spreiding rond de (denkbeeldige) lijn."
      }
      feedback.value = { type: 'error', text: errorMsg }
    } else {
      errorDetected.value = "Hier is het juiste antwoord."
