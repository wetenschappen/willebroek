...
function checkPrediction() {
  ...
  if (predictedStep.value === 'opposite') {
    ...
    const errMsg = hints[Math.min(attemptCount.value - 1, 2)]
...
function runProof(choice) {
  ...
  if (choice === 'opposite') {
    ...
    const errMsg = errorMsgs[Math.min(attemptCount.value - 1, 2)]
    errorDetected.value = errMsg
    feedback.value = {
      type: 'error',
      text: errMsg
        ...
        <div class="flex-col hidden w-full max-w-sm bg-slate-800 border-r border-slate-700 shadow-inner md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div v-if="showReflection" class="p-4 border border-indigo-700 bg-indigo-900/20 rounded-xl animate-fadeIn mb-4">
              <div class="flex items-center gap-2 mb-3">
                <PhPencilSimpleLine class="w-5 h-5 text-amber-400" weight="fill" />
            ...
            <div v-if="!isCorrect && attemptCount > 0 && errorDetected" class="mt-4 p-4 border border-red-800 bg-red-900/30 rounded-xl animate-fadeIn">
              <h4 class="flex items-center gap-2 text-sm font-bold text-red-400 mb-1">
                <PhWarningCircle weight="fill" class="w-4 h-4" />
                Let op!
              </h4>
              <p class="text-sm text-red-300">{{ errorDetected }}</p>
            </div>
          </div>
