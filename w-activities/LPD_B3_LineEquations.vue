...
function checkReflection() {
  ...
  if (reflectAns.value.trim().length > 0) {
    ...
    attemptCount.value++
    errorDetected.value = "Je reflectie is leeg."
    feedback.value = { type: 'error', text: 'Probeer wat uitgebreider te antwoorden.' }
  }
}
    ...
    if (selectedExpr.value === selectedAns.value) {
      ...
      if (exprItem && ansItem) {
        ...
        if (ansItem.ans === normalVector) {
          errorDetected.value = "Let op: je hebt de normaalvector gekozen, niet de richtingsvector."
          feedback.value = {
            type: 'error',
          ...
          errorDetected.value = "Deze koppeling is onjuist."
          feedback.value = {
            type: 'error',
