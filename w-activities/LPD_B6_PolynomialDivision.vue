      ...
      <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-amber-50 border border-amber-200 rounded-xl p-6 mb-6">
          ...
          <div class="space-y-3 text-slate-700">
            ...
            <p class="mt-2 font-bold text-amber-700">Rest = 7. Het quotient: 2x² + 1x + 6.</p>
          </div>
        </div>
    ...
    <div v-if="showPredictionGate&&!predictionMade" class="absolute inset-0 z-20 bg-white overflow-y-auto p-8">
      <div class="max-w-xl mx-auto text-center">
        ...
        <div class="flex flex-col gap-3">
          <button @click="handlePrediction('2')" class="p-4 border-2 border-amber-200 rounded-xl hover:bg-amber-50 hover:border-amber-400 font-bold text-slate-700 transition-all">2 (de coefficient van x²)</button>
      ...
      <main class="flex flex-1 overflow-hidden">
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div v-if="currentLevel.hornerSteps && currentInternalLevel >= 1" class="p-4 mt-4 bg-amber-50 border border-amber-200 rounded-lg text-xs">
              <p class="font-bold text-amber-700 mb-1">Tip: Horner-schema</p>
              <p class="text-amber-600">Noteer coefficienten [{{ currentLevel.poly.coeffs.join(', ') }}] en a={{ currentLevel.divisor.k }}. Breng {{ currentLevel.poly.coeffs[0] }} naar beneden.</p>
            </div>
            ...
            <div v-if="showReflection&&!reflectionDone" class="p-4 mt-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
              ...
              <p class="text-xs text-amber-700 mb-3"><strong>Waarom</strong> is Horner efficienter dan de staartdeling? Wat betekent het als de rest = 0?</p>
            ...
            <div v-if="showLPDEvidence&&!lpdDone" class="p-4 mt-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
              <h4 class="font-bold text-amber-800 mb-3 text-sm">LPD B6: Bewijs van beheersing</h4>
              <p class="text-xs text-amber-700 mb-3">Je hebt bewezen dat je Horner kan toepassen. Vul aan: "Ik bepaal a uit d(x)=x-a. In het schema noteer ik de ____, breng de eerste naar ____, en herhaal: ____ met a en ____ bij de volgende. De laatste waarde is de ____."</p>
