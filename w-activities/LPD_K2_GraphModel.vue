    ...
    <div v-if="showPredictionGate&&!predictionMade" class="absolute inset-0 z-20 bg-white overflow-y-auto p-8">
      <div class="max-w-xl mx-auto text-center">
        ...
        <div class="flex flex-col gap-3">
          <button @click="handlePrediction('graad')" class="p-4 border-2 border-amber-200 rounded-xl hover:bg-amber-50 hover:border-amber-400 font-bold text-slate-700 transition-all">graad van de knoop</button>
        ...
        <div class="flex-col hidden w-full max-w-sm bg-white border-r border-slate-200 md:flex z-10">
          <div ref="mainArea" tabindex="-1" class="flex-1 p-6 overflow-y-auto">
            ...
            <div class="p-6 border-t border-slate-200 bg-slate-50 rounded-xl space-y-4">
              <template v-if="currentLevel.type === 'interpret'">
                ...
                <div class="flex flex-col gap-2">
                  <button v-for="opt in currentLevel.options" :key="opt" @click="userChoice=opt; isChecked=false" class="p-4 border-2 rounded-xl font-bold text-left transition-all" :class="userChoice===opt?'border-indigo-500 bg-amber-50 text-amber-700':'border-slate-200 hover:border-amber-300 bg-white text-slate-700'">{{ opt }}</button>
                </div>
              </template>
              <template v-else>
            ...
            <div v-if="showReflection&&!reflectionDone" class="p-4 mt-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
              ...
              <p class="text-xs text-amber-700 mb-3"><strong>Waarom</strong> is het handig om een sociaal netwerk als een graaf voor te stellen? Wat zie je makkelijker in een graaf dan in een tabel?</p>
            ...
            <div v-if="showLPDEvidence&&!lpdDone" class="p-4 mt-4 bg-amber-50 border border-amber-200 rounded-xl animate-fadeIn">
              <h4 class="font-bold text-amber-800 mb-3 text-sm">LPD K2: Bewijs van beheersing</h4>
              <p class="text-xs text-amber-700 mb-3">Je hebt bewezen dat je een graaf kan interpreteren als model van een reele situatie. Vul aan: "In een graaf stellen de _____ de objecten voor en de _____ de relaties. De _____ van een knoop zegt hoeveel relaties die knoop heeft."</p>
